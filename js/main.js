  ...
  const STORY_FINAL = [
    ...
    { text: 'めが まっかに ひかる。ひびわれた からだから ちからが あふれて ── ★タンクンが きょうらんした。★',
      art: (c, W, H) => {
        ...
        stNamePlate(c, W, H, 'きょうらん タンクン', 'ウイルスに のっとられた', '#ff8a80');
      } },

    /* 14 ★ほうしんが なかまを むく（てきに なった）*/
    { text: 'ゆっくりと ほうしんが うごいた。── ★むいた さきは、なかまたち だった。★',
      art: (c, W, H) => {
        stPlain(c, W, H, true);
        stFlash(c, W, H, '#2a1020');
        stChar(c, 'tankun_kyoran', W * 0.22, H * 0.95, H * 0.44);
        stChar(c, 'purio',   W * 0.58, H * 0.96, H * 0.22);
        stChar(c, 'kabekun', W * 0.72, H * 0.96, H * 0.24);
        stChar(c, 'akibou',  W * 0.88, H * 0.96, H * 0.28);
      } },

    /* 15 こえが とどかない */
    { text: '「タンクン、やめて！　わたしだよ、ぷりおだよ！」　……その めには、もう だれも うつって いなかった。',
      art: (c, W, H) => {
        stPlain(c, W, H, true);
        stRays(c, W, H, W * 0.26, H * 0.66, '#ff3b3b', 16);
        stChar(c, 'tankun_kyoran', W * 0.26, H * 0.95, H * 0.46);
        stChar(c, 'purio', W * 0.68, H * 0.96, H * 0.34);
      } },

    /* 16 ひとふりで いわが くだけた */
    { text: 'ひとふりで いわが くだけた。★まえより ずっと つよい。★　みんな、よけるので せいいっぱい だった。',
      art: (c, W, H) => {
        stPlain(c, W, H, true);
        stFlash(c, W, H, '#3a1424');
        stDebris(c, W, H, 20);
        stChar(c, 'tankun_kyoran', W * 0.34, H * 0.94, H * 0.50,
               { t: 0.4, moving: false, atk: 0.5, hpRatio: 1, hpRate: 1, roll: 0.35 });
        stChar(c, 'kabekun', W * 0.78, H * 0.97, H * 0.20);
        stChar(c, 'purio',   W * 0.90, H * 0.97, H * 0.18);
      } },

    /* 17 はしりさって いく */
    { text: 'そして タンクンは ほえると、だれも いない ★きたの ほう★ へ はしりさって いって しまった。',
      art: (c, W, H) => {
        stPlain(c, W, H, true);
        stChar(c, 'tankun_kyoran', W * 0.80, H * 0.92, H * 0.22);
        stChar(c, 'purio',   W * 0.20, H * 0.96, H * 0.22);
        stChar(c, 'kabekun', W * 0.32, H * 0.96, H * 0.24);
        stChar(c, 'nekos_fight', W * 0.08, H * 0.94, H * 0.40);
      } },

    /* 18 かならず つれもどす */
    { text: '「……あれは もう てき じゃ。」ネコスは そう いって、すぐに くびを ふった。「いや。★まだ タンクンじゃ。かならず つれもどす。★」',
      art: (c, W, H) => {
        stPlain(c, W, H, true);
        stRays(c, W, H, W * 0.40, H * 0.50, '#ffe8a8', 16);
        stChar(c, 'nekos_fight', W * 0.34, H * 0.94, H * 0.58);
        stChar(c, 'purio',   W * 0.68, H * 0.96, H * 0.24);
        stChar(c, 'kabekun', W * 0.84, H * 0.96, H * 0.26);
      } },

    /* 19 つづく */
    { text: 'たつまきは おわった。けれど、こんどは ★なかまを とりもどす たび★ が はじまる。── ★つぎの しょうへ つづく。★',
      art: (c, W, H) => {
        stDawn(c, W, H);
