...
/* オレマッチくん ── マッチくんの だい2けいたい。
   おれめが 2つに ふえて、もっと ぐにゃりと まがって います。
   たおれない ように ひっしで はしり、てに もった ライターで ひを つけて
   こうげき します。からだは あいかわらず ほそい。                        */
function drawOrematch(ctx, s) {
  const a = s.atk;
  const run = Math.sin(s.t * 16) * (s.moving ? 1 : 0);   // はやい あしどり
  const wob = Math.sin(s.t * 6.5) * 0.12;                // たおれそうな ふらつき
  ...
  function zig(y) {
    ...
    ctx.beginPath();
    ctx.moveTo(-4.5, y);
    ctx.lineTo(-1.5, y - 4); ctx.lineTo(1, y + 1); ctx.lineTo(3.5, y - 4); ctx.lineTo(4.5, y);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle = '#a8814c'; ctx.lineWidth = 1.4; ctx.stroke();
  }
  function stick(h, col) {
    ctx.fillStyle = col;
    roundRect(ctx, -4.5, -h, 9, h + 2, 3); ctx.fill();
    ctx.strokeStyle = '#a8814c'; ctx.lineWidth = 1.6;
    roundRect(ctx, -4.5, -h, 9, h + 2, 3); ctx.stroke();
  }
  ...
  /* --- あし（おおきく ふりだして はしる）--- */
  ctx.strokeStyle = '#c9a06a'; ctx.lineWidth = 4.2; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-2, -18);
  ctx.quadraticCurveTo(-9 + run * 9, -11, -13 + run * 15, 0);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(3, -18);
  ctx.quadraticCurveTo(10 - run * 9, -11, 14 - run * 15, 0);
  ctx.stroke();
  /* はしって いる けむり */
  if (s.moving) {
    ctx.fillStyle = 'rgba(200,190,175,.35)';
    for (let i = 0; i < 2; i++) {
      const r = 3 + i * 2 + Math.abs(run) * 2;
      ctx.beginPath(); ctx.arc(-18 - i * 9, -3 - i * 2, r, 0, Math.PI * 2); ctx.fill();
    }
  }

  /* --- 1だんめ：まえに かたむく --- */
  ctx.translate(0, -17);
  ctx.rotate(-0.20 + run * 0.05);
  stick(20, '#d9b483');
  zig(-20);

  /* --- 2だんめ：ぎゃくに おれる（★おれめ その2）--- */
  ctx.translate(0, -20);
  ctx.rotate(0.40 + Math.sin(s.t * 6.5) * 0.06);
  stick(20, '#dfbe8e');

  /* ===== うでは この だんに つける（あたまより したに なる ように）===== */
  /* ひだりうで：バランスを とる ため うしろに ふりまわす */
  ctx.strokeStyle = '#c9a06a'; ctx.lineWidth = 3.6; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-4, -13);
  ctx.quadraticCurveTo(-16, -14 - run * 7, -23, -5 - run * 11);
  ctx.stroke();

  /* みぎうで：ライターを もって、こうげきの ときに まえへ つきだす */
  const swing = (a >= 0) ? (0.55 - a * 1.15) : (0.75 + run * 0.22);
  ctx.save();
  ctx.translate(4, -13);
  ctx.rotate(swing);
  ctx.strokeStyle = '#c9a06a'; ctx.lineWidth = 3.6;
  ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(17, 0); ctx.stroke();

  /* ★ライター（あかい ボディ＋ぎんの ふた）*/
  ctx.save();
  ctx.translate(19, 0);
  ctx.rotate(-swing - 0.15);            // ライターは いつも たてむきに もつ
  ctx.fillStyle = '#d8322a';
  roundRect(ctx, -3.5, -5, 7, 11, 2); ctx.fill();
  ctx.strokeStyle = '#7e1a13'; ctx.lineWidth = 1.2;
  roundRect(ctx, -3.5, -5, 7, 11, 2); ctx.stroke();
  ctx.fillStyle = '#cfd8dc';
  roundRect(ctx, -3.2, -8, 6.4, 3.5, 1); ctx.fill();
  ctx.strokeStyle = '#78909c'; ctx.lineWidth = 1; ctx.stroke();
  /* ライターの ひ（こうげきちゅうは おおきく もえる）*/
  const lf = (a >= 0) ? (0.6 + a * 1.6) : 0.5;
  for (let i = 0; i < 2; i++) {
    const h = (12 - i * 4.5) * lf;
    const w = 3.4 - i * 1.2;
    ctx.fillStyle = i === 0 ? 'rgba(255,138,0,.95)' : 'rgba(255,241,118,.95)';
    ctx.beginPath();
    ctx.moveTo(-w, -8);
    ctx.quadraticCurveTo(-w + Math.sin(s.t * 17) * 1.2, -8 - h * 0.6, 0, -8 - h);
    ctx.quadraticCurveTo(w + Math.sin(s.t * 17) * 1.2, -8 - h * 0.6, w, -8);
    ctx.closePath(); ctx.fill();
  }
  ctx.restore();
  ctx.restore();
  ...
  /* --- 3だんめ：また もどる ように おれる ＋ あたま --- */
  ctx.translate(0, -20);
  ctx.rotate(-0.26 + Math.sin(s.t * 6.5 + 1.2) * 0.05);
  stick(16, '#e0c092');

  /* あたま（あかい やくひん）*/
  const hg = ctx.createRadialGradient(-3, -20, 2, 0, -17, 14);
  hg.addColorStop(0, '#ff8a65');
  hg.addColorStop(1, '#b9251c');
  ctx.fillStyle = hg;
  ellipse(ctx, 0, -17, 10.5, 12.5); ctx.fill();
  ctx.strokeStyle = '#7e1a13'; ctx.lineWidth = 1.6;
  ellipse(ctx, 0, -17, 10.5, 12.5); ctx.stroke();

  /* かお：ひっしで はしって いる（＞＜ の め と あせ）*/
  ctx.strokeStyle = '#2b1b12'; ctx.lineWidth = 1.7; ctx.lineCap = 'round';
  for (const [ex, d] of [[-4.2, 1], [4.2, -1]]) {
    ctx.beginPath();
    ctx.moveTo(ex - 2.4 * d, -22); ctx.lineTo(ex + 2.0 * d, -19.5);
    ctx.lineTo(ex - 2.4 * d, -17); ctx.stroke();
  }
  ctx.beginPath(); ctx.arc(0, -11.5, 3.2, Math.PI * 1.1, Math.PI * 1.9); ctx.stroke();
  ctx.fillStyle = 'rgba(120,200,255,.9)';                     // あせ
  ctx.beginPath();
  ctx.ellipse(9.5, -23 + Math.sin(s.t * 7) * 1.5, 1.8, 2.6, 0.3, 0, Math.PI * 2); ctx.fill();
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(-8 + i * 3, -27);
    ctx.quadraticCurveTo(-4 + i * 2 + f, -27 - h * 0.6, f * 0.5, -27 - h);
    ctx.quadraticCurveTo(6 - i * 2 + f, -27 - h * 0.6, 8 - i * 3, -27);
    ctx.closePath(); ctx.fill();
  }
...
/* ============================================================
   ★デザインあん★ ドズル社（スキンに にせた ブロックちょう）
   まだ ゲームには でて きません（ENEMIES に いれて いない ため）。
   スティーブ・クリーパー・アイアンゴーレムと おなじ マイクラふうの
   かきかたに そろえて います。
   ============================================================ */

/* ---- きょうつう ぶひん：あし ---- */
function mcLeg(ctx, x, w, h, pants, shoe) {
  ctx.fillStyle = pants; ctx.fillRect(x, -h, w, h);
  if (shoe) { ctx.fillStyle = shoe; ctx.fillRect(x, -5, w, 5); }
}
/* ---- きょうつう ぶひん：うで（つけね で まわす）---- */
function mcArm(ctx, px, py, rot, sleeve, hand, len) {
  ctx.save(); ctx.translate(px, py); ctx.rotate(rot);
  ctx.fillStyle = sleeve; ctx.fillRect(0, -5, len - 9, 10);
  ctx.fillStyle = hand;   ctx.fillRect(len - 10, -5, 10, 10);
  ctx.restore();
}
/* ---- うでの ふりかた（あるく／こうげき）----
   ★うでは 0 が「まよこ」なので、ふだんは 1.45（＝したに たれる）に します。
     ものを もって いる こは、みえる ように まえに かまえます（hold）。 */
function mcSwing(s, hold) {
  const a = s.atk;
  const walk = Math.sin(s.t * 5) * (s.moving ? 1 : 0);
  const idle = hold ? 0.55 : 1.45;
  return {
    walk: walk,
    back:  (a >= 0) ? 1.75 : (1.45 + walk * 0.40),
    front: (a >= 0) ? (-0.85 + a * 1.75) : (idle - walk * 0.40),
  };
}
...
function drawOlaf(ctx, s) {
  ...
  ctx.save(); ctx.strokeStyle = '#b9cddd'; ctx.lineWidth = 1.4;
  ...
  for (let i = 0; i < 4; i++) {
    const t = s.t * 1.3 + i * 1.9;
    ctx.fillRect(-26 + i * 16 + Math.sin(t) * 5, -104 + ((t * 13) % 100), 3, 3);
  }
  ctx.restore();
}
...
function nekoHead(ctx, s, opt) {
  opt = opt || {};
  const a = s.atk;
  const FUR = opt.fur || '#8a8a8a', FUR2 = opt.fur2 || '#6f6f6f',
        FACE = opt.face || '#efe7d8';
  /* みみ */
  ctx.fillStyle = FUR;
  ctx.fillRect(-13, -88, 8, 9); ctx.fillRect(5, -88, 8, 9);
  ctx.fillStyle = '#f2b0c0';
  ctx.fillRect(-11, -85, 4, 5); ctx.fillRect(7, -85, 4, 5);
  /* あたま */
  ctx.fillStyle = FUR;  ctx.fillRect(-13, -79, 26, 27);
  ctx.fillStyle = FUR2; ctx.fillRect(-13, -79, 5, 27);
  /* かおの しろい ところ */
  ctx.fillStyle = FACE; ctx.fillRect(-4, -72, 17, 20);
  /* め */
  ctx.fillStyle = '#1c1c22';
  ctx.fillRect(-1, -68, 5, 6); ctx.fillRect(8, -68, 5, 6);
  if (opt.glasses) {                     /* はかせの メガネ */
    ctx.strokeStyle = '#2b2b33'; ctx.lineWidth = 2;
    ctx.strokeRect(-3, -70, 9, 10); ctx.strokeRect(6, -70, 9, 10);
    ctx.fillStyle = 'rgba(190,230,255,.45)';
    ctx.fillRect(-3, -70, 9, 10); ctx.fillRect(6, -70, 9, 10);
    ctx.fillStyle = '#1c1c22';
    ctx.fillRect(-1, -68, 4, 5); ctx.fillRect(8, -68, 4, 5);
  }
  /* はな と くち */
  ctx.fillStyle = '#e2899a'; ctx.fillRect(4, -60, 5, 4);
  ctx.fillStyle = '#3a3a42'; ctx.fillRect(2, -56, 9, (a >= 0) ? 5 : 2);
  /* ヒゲ */
  ctx.fillStyle = 'rgba(250,250,250,.85)';
  ctx.fillRect(11, -60, 12, 2); ctx.fillRect(11, -55, 12, 2);
  if (opt.beard) {                        /* おじいちゃんの しろい ひげ */
    ctx.fillStyle = '#f0f0f0';
    ctx.fillRect(0, -53, 13, 6);
  }
}

/* ネコおじ ① ふつう（グレーの ねこ）*/
function drawNekooji(ctx, s) {
  const w = mcSwing(s);
  const FUR = '#8a8a8a', FUR2 = '#6f6f6f', WHITE = '#f2f2ee';
  ctx.save();
  /* しっぽ */
  ctx.fillStyle = FUR2;
  ctx.fillRect(-22, -44 + Math.sin(s.t * 3) * 3, 10, 5);
  ctx.fillRect(-26, -56 + Math.sin(s.t * 3) * 3, 5, 14);
  mcLeg(ctx, -11 + w.walk * 3, 10, 22, FUR, WHITE);
  mcLeg(ctx,   2 - w.walk * 3, 10, 22, FUR, WHITE);
  mcArm(ctx, -12, -46, w.back, FUR2, WHITE, 24);
  /* からだ（むねが しろい）*/
  ctx.fillStyle = FUR;   ctx.fillRect(-13, -50, 26, 28);
  ctx.fillStyle = WHITE; ctx.fillRect(-5, -50, 12, 24);
  nekoHead(ctx, s);
  mcArm(ctx, 12, -46, w.front, FUR, WHITE, 24);
  ctx.restore();
}

/* ネコおじ ② スーツ */
function drawNekoojiSuit(ctx, s) {
  const w = mcSwing(s);
  const FUR = '#8a8a8a', SUIT = '#1e1e24', SUIT2 = '#141418', WHITE = '#f2f2ee';
  ctx.save();
  ctx.fillStyle = '#6f6f6f';
  ctx.fillRect(-22, -44 + Math.sin(s.t * 3) * 3, 10, 5);
  ctx.fillRect(-26, -56 + Math.sin(s.t * 3) * 3, 5, 14);
  mcLeg(ctx, -11 + w.walk * 3, 10, 22, SUIT, '#0e0e12');
  mcLeg(ctx,   2 - w.walk * 3, 10, 22, SUIT, '#0e0e12');
  mcArm(ctx, -12, -46, w.back, SUIT2, FUR, 24);
  /* くろい スーツ */
  ctx.fillStyle = SUIT;  ctx.fillRect(-13, -52, 26, 30);
  ctx.fillStyle = WHITE; ctx.fillRect(-4, -52, 8, 15);
  ctx.fillStyle = '#9aa3ad'; ctx.fillRect(-2, -50, 4, 14);
  ctx.fillStyle = SUIT2; ctx.fillRect(-13, -52, 6, 30); ctx.fillRect(7, -52, 6, 30);
  nekoHead(ctx, s, { face: '#e8e0d0' });
  mcArm(ctx, 12, -46, w.front, SUIT, FUR, 24);
  ctx.restore();
}

/* ネコおじ ③ おじいちゃんモード */
function drawNekoojiOld(ctx, s) {
  const w = mcSwing(s, true);
  const FUR = '#a89f92', COAT = '#c9a03c', COAT2 = '#a8842c', PANTS = '#9a9a94';
  ctx.save();
  ctx.fillStyle = '#8f877c';
  ctx.fillRect(-22, -42 + Math.sin(s.t * 2) * 2, 10, 5);
  ctx.fillRect(-26, -54 + Math.sin(s.t * 2) * 2, 5, 14);
  mcLeg(ctx, -11 + w.walk * 2, 10, 22, PANTS, '#7e7e78');
  mcLeg(ctx,   2 - w.walk * 2, 10, 22, PANTS, '#7e7e78');
  mcArm(ctx, -12, -46, w.back * 0.6, COAT2, FUR, 24);
  /* からしいろの はおり */
  ctx.fillStyle = '#efe7d5'; ctx.fillRect(-13, -50, 26, 28);
  ctx.fillStyle = COAT;
  ctx.fillRect(-13, -50, 10, 28); ctx.fillRect(3, -50, 10, 28);
  ctx.fillStyle = COAT2; ctx.fillRect(-13, -50, 26, 4);
  nekoHead(ctx, s, { fur: '#a89f92', fur2: '#8f877c', face: '#efe6d6', beard: true });
  mcArm(ctx, 12, -46, w.front * 0.6, COAT, FUR, 24);
  /* つえ */
  ctx.save();
  ctx.translate(12, -46); ctx.rotate(w.front * 0.6); ctx.translate(24, 0); ctx.rotate(-w.front * 0.6);
  ctx.fillStyle = '#8a6234'; ctx.fillRect(-2, -6, 4, 50);
...
/* ネコおじ ④ はかせ（はくい）*/
function drawNekoojiDr(ctx, s) {
  const w = mcSwing(s, true);
  const FUR = '#8a8a8a', COAT = '#f4f6f8', COAT2 = '#d9dee3', PANTS = '#5b6570';
  ctx.save();
  ctx.fillStyle = '#6f6f6f';
  ctx.fillRect(-22, -44 + Math.sin(s.t * 3) * 3, 10, 5);
  ctx.fillRect(-26, -56 + Math.sin(s.t * 3) * 3, 5, 14);
  mcLeg(ctx, -11 + w.walk * 3, 10, 22, PANTS, '#424a52');
  mcLeg(ctx,   2 - w.walk * 3, 10, 22, PANTS, '#424a52');
  mcArm(ctx, -12, -46, w.back, COAT2, FUR, 24);
  /* しろい はくい */
  ctx.fillStyle = '#7fb8c9'; ctx.fillRect(-13, -50, 26, 28);
  ctx.fillStyle = COAT;
  ctx.fillRect(-13, -52, 10, 32); ctx.fillRect(3, -52, 10, 32);
  ctx.fillStyle = COAT2; ctx.fillRect(-13, -52, 26, 4);
  ctx.fillStyle = COAT2; ctx.fillRect(4, -36, 8, 8);        // ポケット
  nekoHead(ctx, s, { glasses: true });
  mcArm(ctx, 12, -46, w.front, COAT, FUR, 24);
  /* フラスコ */
  ctx.save();
  ctx.translate(12, -46); ctx.rotate(w.front); ctx.translate(25, 0); ctx.rotate(-w.front);
  ctx.fillStyle = '#cfe8f5'; ctx.fillRect(-6, -14, 12, 16);
  ctx.fillRect(-3, -20, 6, 7);
  ctx.fillStyle = '#7ce07c'; ctx.fillRect(-5, -7, 10, 8);
  ctx.fillStyle = 'rgba(124,224,124,.6)';
  ctx.fillRect(-2, -26 - Math.abs(Math.sin(s.t * 6)) * 4, 3, 4);
  ctx.restore();
  ctx.restore();
}

/* ==========================================================================
   ★小惑星ぷりぷりの キャラクター（ごうのすけくんの えを そのまま）
   ========================================================================== */

/* アリ ── ごうのすけくんの え：
     あおい ほそながい からだが よこに のび、そこから したに とがった あしが
     3ぼん。みぎはしに おおきな まるい かお。かおには たてぼうの め 2つと
     にっこりした くち。トゲありは からだの うえに するどい トゲが 3ぼん。

   toge = true で「トゲありアリ」に なります。                            */
function drawAntBase(ctx, s, toge) {
  const a = s.atk;
  const walk = s.moving ? Math.sin(s.t * 9) : 0;
  const bob  = s.moving ? Math.abs(Math.sin(s.t * 9)) * 2 : 0;
  const BLUE = toge ? '#2f93cc' : '#59b6e2';
  const DARK = toge ? '#2179ab' : '#3f97c2';

  ctx.save();
  ctx.translate(0, -bob);

  const bodyY = -44;                 // からだの まんなかの たかさ
  const xL = -62, xR = 4;            // からだの ひだりはし・みぎはし
  const legX = [xL + 12, xL + 32, xL + 52];

  /* --- あし＝したむきの ふとい さんかく 3ぼん --- */
  ctx.fillStyle = DARK;
  for (let i = 0; i < 3; i++) {
    const kick = Math.sin(s.t * 9 + i * 2.1) * 6;
    ctx.beginPath();
    ctx.moveTo(legX[i] - 11, bodyY + 4);
    ctx.lineTo(legX[i] + 11, bodyY + 4);
    ctx.lineTo(legX[i] + kick, 0);     // つまさきが ちめんに つく
    ctx.closePath();
    ctx.fill();
  }

  /* --- トゲ＝うわむきの ふとい さんかく 3ぼん（トゲありだけ）--- */
  if (toge) {
    ctx.fillStyle = BLUE;
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
      ctx.moveTo(legX[i] - 12, bodyY - 4);
      ctx.lineTo(legX[i] + 12, bodyY - 4);
      ctx.lineTo(legX[i] + 1, bodyY - 66);   // ★するどく ながい
      ctx.closePath();
      ctx.fill();
    }
    ...
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
      ctx.moveTo(legX[i] - 4, bodyY - 4);
      ctx.lineTo(legX[i] + 1, bodyY - 4);
      ctx.lineTo(legX[i] + 1, bodyY - 58);
      ctx.closePath();
      ctx.fill();
    }
  }

  /* --- からだ（よこに ながい ふとい ぼう。ひだりはしは しっぽ）--- */
  ctx.fillStyle = BLUE;
  ctx.beginPath();
  ctx.moveTo(xL - 20, bodyY + 1);      // しっぽの さき（とがって いる）
  ctx.lineTo(xL, bodyY - 13);
  ctx.lineTo(xR, bodyY - 13);
  ctx.lineTo(xR, bodyY + 10);
  ctx.lineTo(xL, bodyY + 10);
  ctx.closePath();
  ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.22)';
  ctx.fillRect(xL, bodyY - 13, xR - xL, 5);

  /* --- かお（おおきな まる。からだに すこし かぶさる）--- */
  const hx = 26, hy = bodyY - 6, hr = 30;
  ctx.fillStyle = '#eceaf1';
  ctx.beginPath(); ctx.arc(hx, hy, hr, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = '#2b2b33'; ctx.lineWidth = 2.8;
  ...
  for (const d of [-10, 7]) {
    ctx.beginPath();
    ctx.moveTo(hx + d, hy - 13);
    ctx.lineTo(hx + d, hy - 1);
    ctx.stroke();
  }
  /* くち（ゆるやかな にっこり）。こうげき ちゅうは あける */
  ctx.lineWidth = 2.8;
  if (a >= 0) {
    ctx.fillStyle = '#2b2b33';
    ctx.beginPath(); ctx.ellipse(hx - 1, hy + 11, 8, 7, 0, 0, Math.PI * 2); ctx.fill();
  } else {
    ctx.beginPath();
    ctx.arc(hx - 1, hy + 2, 13, 0.22 * Math.PI, 0.78 * Math.PI);
    ctx.stroke();
  }
  ctx.restore();
}
...
/* ぷりぷりくん ── ごうのすけくんの え：
     はだいろの おおきな まんまる（まんなかに たての せんが 1ぽん）。
     そこから ほそい うでが 2ほん よこに のび、したには ほそい あしが 2ほん。
     かおは ない。みずぞくせいの おおボス。                                 */
function drawPuripurikun(ctx, s) {
  const a = s.atk;
  const step = s.moving ? Math.sin(s.t * 2.6) : 0;
  const squash = (a >= 0) ? Math.sin(a * Math.PI) : 0;   // こうげきで ぷりんと はねる
  const bob = Math.abs(step) * 3 + squash * 12;

  ctx.save();
  ctx.translate(0, -bob);

  const cy = -84;                          // からだの まんなか
  const R  = 52 * (1 + squash * 0.10);     // ふたつの やまの おおきさ
  const dx = 27 * (1 + squash * 0.14);     // やまと やまの あいだ

  /* --- あし 2ほん（ほそながい。ちいさな へらの あしさき）--- */
  ...
  for (const d of [-1, 1]) {
    const kick = step * d * 8;
    ctx.beginPath();
    ctx.moveTo(d * 22, cy + R * 0.70);
    ctx.quadraticCurveTo(d * 40, cy + R + 30, d * 36 + kick, -8);
    ctx.stroke();
    ctx.beginPath();
    ctx.ellipse(d * 36 + kick + d * 7, -4, 11, 5.5, d * 0.32, 0, Math.PI * 2);
    ctx.fillStyle = '#1e1e22'; ctx.fill();
  }

  /* --- からだ：まるが ふたつ ならんだ「ぷりぷり」の かたち --- */
  const bg = ctx.createRadialGradient(-dx, cy - R * 0.45, R * 0.15, 0, cy, R * 2.0);
  bg.addColorStop(0, '#f7e6c2');
  bg.addColorStop(0.55, '#e8ce9d');
  bg.addColorStop(1, '#cdac77');
  ctx.fillStyle = bg;
  ctx.beginPath();
  ctx.arc(-dx, cy, R, 0, Math.PI * 2);
  ctx.arc( dx, cy, R, 0, Math.PI * 2);
  ctx.fill();
  /* ふたつの まるの あいだを うめる。ただし うえと したは うめない ので、
     まんなかが すこし へこんだ「ぷりぷり」の かたちに なります */
  ctx.beginPath();
  ctx.moveTo(-dx, cy - R * 0.80);
  ctx.lineTo( dx, cy - R * 0.80);
  ctx.lineTo( dx, cy + R * 0.74);
  ctx.lineTo(-dx, cy + R * 0.74);
  ctx.closePath();
  ctx.fill();

  /* まんなかの たてせん（ごうのすけくんの えの いちばんの とくちょう）*/
  ctx.strokeStyle = '#2b2b30';
  ctx.lineWidth = 4.5;
  ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(2, cy - R * 0.98);
  ctx.quadraticCurveTo(-5, cy + R * 0.1, 1, cy + R * 0.86);
  ctx.stroke();

  /* --- うで 2ほん（ほそくて ひょろっと して いる）--- */
  ctx.strokeStyle = '#1e1e22';
  ctx.lineWidth = 4.5;
  const sw = (a >= 0) ? (1 - squash) : 0;
  const ax = dx + R * 0.86;
  /* ひだりうで：そとに のびて さきが ちいさく まがる */
  ctx.beginPath();
  ctx.moveTo(-ax + 6, cy - R * 0.35);
  ctx.quadraticCurveTo(-ax - 40, cy - R * 0.5 + sw * 22, -ax - 34, cy + 18 + sw * 28);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(-ax - 34, cy + 18 + sw * 28);
  ctx.lineTo(-ax - 46, cy + 26 + sw * 28);
  ctx.stroke();
  /* みぎうで：さきが ゆび 2ほんに わかれて いる */
  ctx.beginPath();
  ctx.moveTo(ax - 6, cy - R * 0.35);
  ctx.quadraticCurveTo(ax + 42, cy - R * 0.55 + sw * 26, ax + 34, cy + 14 + sw * 32);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(ax + 34, cy + 14 + sw * 32);
  ctx.lineTo(ax + 48, cy + 8 + sw * 32);
  ctx.moveTo(ax + 34, cy + 14 + sw * 32);
  ctx.lineTo(ax + 45, cy + 26 + sw * 32);
  ctx.stroke();

  /* --- こうげきの えんしゅつ：みずの わが ひろがる --- */
  if (a >= 0) {
    ctx.strokeStyle = 'rgba(110,195,240,' + (0.8 * (1 - a)).toFixed(2) + ')';
    ctx.lineWidth = 6;
    const rr = 34 + a * 120;
    ctx.beginPath();
    ctx.ellipse(0, -6, rr, rr * 0.30, 0, 0, Math.PI * 2);
    ctx.stroke();
  }
  ctx.restore();
}
...
/* ぷり王プーリッチ ── ぷりおぷりねこの だい3けいたい。
   ごうのすけくんの えの とおり：
     ...
     ・みぎに ながく のびる にっこりした くち、うっすら ほっぺ
     ・くびには くびわ と しかくい ふだ
     ・りょううでを ひろげ、さきは とがった つめ
     ・あしは ギザギザ（Wのかたち）

   ★おおきさは PURIKING_STYLES で きめて います★
     だい2けいたい（ぷりぷりおぷりねこ）は たかさ やく 92px。
     ここは「ひとまわり おおきい」やく 105px に なる ように して あります。 */
const PURIKING_STYLES = {
  /* A：いまの ふんいきの まま、こがらに した もの */
  /* k ＝ ぜんたいの おおきさ。だい2けいたい（たかさ 90）の
     1.17ばい くらい（＝たかさ 105）に なる ように あわせて います。 */
  slim:  { k: 0.90, hy: -73, hr: 25, earLen: 33, bw: 14, bTop: -46, bBot: -9,
           egg: false, armY: -41, armX: 35, armDown: -26, claw: 15, footTop: -12, face: 0.76 },
  /* B：あたまを おおきく、からだを みじかく した かわいいがた */
  chibi: { k: 0.90, hy: -70, hr: 30, earLen: 34, bw: 17, bTop: -42, bBot: -9,
           egg: false, armY: -37, armX: 34, armDown: -22, claw: 14, footTop: -12, face: 0.90 },
  /* C：からだも まるい、いちばん まるっこい かたち */
  round: { k: 0.94, hy: -68, hr: 29, earLen: 32, bw: 20, bTop: -44, bBot: -8,
           egg: true,  armY: -36, armX: 33, armDown: -20, claw: 13, footTop: -11, face: 0.88 },
};
/* ★いま つかって いる かたち */
...
const PURIKING_TEX = {
  ...
  anime: { label: 'アニメ',        ink: '#3a2206', lw: 1.05, outline: '#6b3d0a', ow: 3.0, flat: true, shade: true, gloss: 'small' },
...
function purikingPaint(ctx, path, cx, cy, r, T, s) {
  ...
  /* --- ほんたい --- */
  path();
  if (T.flat)      ctx.fillStyle = '#f7b154';
  else if (T.pastel) {
  ...
  if (T.shade) {
    ...
    ctx.fillStyle = 'rgba(206,122,26,.48)';
    ctx.beginPath();
...
/* ==== ぷり王の「デザインあん」5つ ====
   かたち（chibi）と いろ（オレンジ）と「ねこの おうさま」と いう
   ほんしつは かえず、かおの かきかたと つけたす ものだけを かえて います。 */

/* ② アニメ調：おおきな たまごがたの め、ちいさな さんかくの はな、ω の くち */
function pkFaceAnime(ctx, hy, hr, f, a, INK) {
  /* め（しろめ ＋ くろめ ＋ ハイライト）*/
  for (const d of [-1, 1]) {
    const ex = d * 11 * f, ey = hy - 4 * f;
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.ellipse(ex, ey, 7.4 * f, 9.4 * f, d * 0.08, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = INK; ctx.lineWidth = 1.9 * f;
    ctx.beginPath(); ctx.ellipse(ex, ey, 7.4 * f, 9.4 * f, d * 0.08, 0, Math.PI * 2); ctx.stroke();
    ctx.fillStyle = '#3c2a12';
    ctx.beginPath(); ctx.ellipse(ex + d * 0.8 * f, ey + 1 * f, 4.4 * f, 7 * f, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.arc(ex - d * 1.6 * f, ey - 3.4 * f, 2.2 * f, 0, Math.PI * 2); ctx.fill();
    /* まぶたの せん */
    ctx.strokeStyle = INK; ctx.lineWidth = 2.4 * f; ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(ex - 7.4 * f, ey - 9.4 * f);
    ctx.quadraticCurveTo(ex, ey - 13.6 * f, ex + 7.4 * f, ey - 9.4 * f);
    ctx.stroke();
  }
  /* はな */
  ctx.fillStyle = '#d4713e';
  ctx.beginPath();
  ctx.moveTo(-3.2 * f, hy + 8 * f); ctx.lineTo(3.2 * f, hy + 8 * f); ctx.lineTo(0, hy + 12 * f);
  ctx.closePath(); ctx.fill();
  /* くち（ω）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4 * f; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(0, hy + 12 * f);
  ctx.quadraticCurveTo(-3 * f, hy + (a >= 0 ? 22 : 18) * f, -7 * f, hy + 14 * f);
  ctx.moveTo(0, hy + 12 * f);
  ctx.quadraticCurveTo(3 * f, hy + (a >= 0 ? 22 : 18) * f, 7 * f, hy + 14 * f);
  ctx.stroke();
  /* ほっぺ */
  ctx.fillStyle = 'rgba(232,110,60,.28)';
  for (const d of [-1, 1]) { ctx.beginPath(); ctx.ellipse(d * 21 * f, hy + 10 * f, 7 * f, 4.4 * f, 0, 0, Math.PI * 2); ctx.fill(); }
}

/* ③ キュート：とにかく おおきな キラキラの め、ちいさな くち、おおきな ほっぺ */
function pkFaceCute(ctx, hy, hr, f, a, INK) {
  for (const d of [-1, 1]) {
    const ex = d * 11.5 * f, ey = hy - 1 * f;
    ctx.fillStyle = '#2f2016';
    ctx.beginPath(); ctx.ellipse(ex, ey, 8.6 * f, 10.6 * f, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.arc(ex - d * 2.6 * f, ey - 4.2 * f, 3.4 * f, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(ex + d * 2.8 * f, ey + 3.6 * f, 1.8 * f, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.55)';
    ctx.beginPath(); ctx.ellipse(ex, ey + 6.6 * f, 4.6 * f, 1.8 * f, 0, 0, Math.PI * 2); ctx.fill();
  }
  /* ちいさな ω の くち */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2 * f; ctx.lineCap = 'round';
  const oy = hy + (a >= 0 ? 15 : 13) * f;
  ctx.beginPath();
  ctx.moveTo(-4.4 * f, oy); ctx.quadraticCurveTo(-2.2 * f, oy + 4 * f, 0, oy);
  ctx.quadraticCurveTo(2.2 * f, oy + 4 * f, 4.4 * f, oy);
  ctx.stroke();
  /* おおきな ほっぺ */
  ctx.fillStyle = 'rgba(240,120,90,.34)';
  for (const d of [-1, 1]) { ctx.beginPath(); ctx.ellipse(d * 21 * f, hy + 9 * f, 8.6 * f, 6 * f, 0, 0, Math.PI * 2); ctx.fill(); }
}

/* ④ ほんものの ねこ：アーモンドがたの め、たてに ほそい ひとみ、ひげ */
function pkFaceReal(ctx, hy, hr, f, a, INK) {
  for (const d of [-1, 1]) {
    const ex = d * 12 * f, ey = hy - 3 * f;
    ctx.fillStyle = '#f6e07a';
    ctx.beginPath(); ctx.ellipse(ex, ey, 8.2 * f, 6 * f, d * 0.22, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#2a1d0c';
    ctx.beginPath(); ctx.ellipse(ex, ey, 1.9 * f, 5.4 * f, d * 0.22, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.8)';
    ctx.beginPath(); ctx.arc(ex - d * 2.6 * f, ey - 2.4 * f, 1.5 * f, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = INK; ctx.lineWidth = 1.8 * f;
    ctx.beginPath(); ctx.ellipse(ex, ey, 8.2 * f, 6 * f, d * 0.22, 0, Math.PI * 2); ctx.stroke();
  }
  /* はな（さんかく）と ω の くち */
  ctx.fillStyle = '#e0857f';
  ctx.beginPath();
  ctx.moveTo(-3.6 * f, hy + 7 * f); ctx.lineTo(3.6 * f, hy + 7 * f); ctx.lineTo(0, hy + 11.5 * f);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.1 * f; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0, hy + 11.5 * f); ctx.lineTo(0, hy + 14 * f); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(0, hy + 14 * f);
  ctx.quadraticCurveTo(-3.4 * f, hy + (a >= 0 ? 23 : 19) * f, -8 * f, hy + 15 * f);
  ctx.moveTo(0, hy + 14 * f);
  ctx.quadraticCurveTo(3.4 * f, hy + (a >= 0 ? 23 : 19) * f, 8 * f, hy + 15 * f);
  ctx.stroke();
  /* ひげ（りょうがわ 3ぼんずつ）*/
  ctx.strokeStyle = 'rgba(70,48,26,.75)'; ctx.lineWidth = 1.5 * f;
  for (const d of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      const y0 = hy + (6 + i * 5) * f;
      ctx.beginPath();
      ctx.moveTo(d * 12 * f, y0);
      ctx.quadraticCurveTo(d * 26 * f, y0 - 3 * f + i * 2 * f, d * 38 * f, y0 - 6 * f + i * 5 * f);
      ctx.stroke();
    }
  }
}

/* ⑤ ふしぎな いきもの：ひかる め、ひたいの もよう、ゆらぐ かげ */
function pkFaceMyst(ctx, hy, hr, f, a, INK) {
  /* ひたいの もよう */
  ctx.strokeStyle = 'rgba(120,70,10,.55)'; ctx.lineWidth = 4.6 * f;
  ctx.beginPath(); ctx.arc(0, hy - 18 * f, 6.4 * f, 0.12 * Math.PI, 0.88 * Math.PI, true); ctx.stroke();
  ctx.strokeStyle = '#fff6c4'; ctx.lineWidth = 2.6 * f;
  ctx.beginPath(); ctx.arc(0, hy - 18 * f, 6.4 * f, 0.12 * Math.PI, 0.88 * Math.PI, true); ctx.stroke();
  ctx.fillStyle = 'rgba(120,70,10,.5)';
  ctx.beginPath(); ctx.arc(0, hy - 18 * f, 3.4 * f, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#fff6c4';
  ctx.beginPath(); ctx.arc(0, hy - 18 * f, 2.4 * f, 0, Math.PI * 2); ctx.fill();
  /* ひかる め（ひとみは なく、ぼうっと ひかる）*/
  for (const d of [-1, 1]) {
    const ex = d * 11.5 * f, ey = hy - 1 * f;
    const g = ctx.createRadialGradient(ex, ey, 0.5, ex, ey, 9 * f);
    g.addColorStop(0, 'rgba(255,255,230,1)');
    g.addColorStop(0.45, 'rgba(255,214,90,.9)');
    g.addColorStop(1, 'rgba(255,170,40,0)');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(ex, ey, 9 * f, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fffdf0';
    ctx.beginPath(); ctx.ellipse(ex, ey, 2.4 * f, 6 * f, 0, 0, Math.PI * 2); ctx.fill();
  }
  /* くち（ほそい みかづき）*/
  ctx.strokeStyle = 'rgba(255,240,190,.95)'; ctx.lineWidth = 2.4 * f; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-9 * f, hy + 13 * f);
  ctx.quadraticCurveTo(0, hy + (a >= 0 ? 24 : 20) * f, 9 * f, hy + 13 * f);
  ctx.stroke();
}

/* ⑥ かおを ととのえた：もとの あじ（たてぼうの め・「ひ」の はな・ながい くち）を
      そのまま、まっすぐ・ひだりみぎ そろえて きれいに かきなおした もの */
function pkFaceTidy(ctx, hy, hr, f, a, INK) {
  ctx.fillStyle = 'rgba(232,110,60,.24)';
  for (const d of [-1, 1]) { ctx.beginPath(); ctx.ellipse(d * 19 * f, hy + 10 * f, 7.6 * f, 4.8 * f, 0, 0, Math.PI * 2); ctx.fill(); }
  /* め：ひだりみぎ そろった たてぼう。さきを まるく、ちょっと ふとく */
  ctx.strokeStyle = INK; ctx.lineWidth = 5 * f; ctx.lineCap = 'round';
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.moveTo(d * 10 * f, hy - 13 * f);
    ctx.lineTo(d * 10 * f, hy + 1 * f);
    ctx.stroke();
  }
  /* はな：まんなかに そろえた「ひ」*/
  ctx.lineWidth = 3 * f;
  ctx.beginPath();
  ctx.moveTo(-1.5 * f, hy + 7 * f);
  ctx.quadraticCurveTo(-1 * f, hy + 15 * f, 2 * f, hy + 15.5 * f);
  ctx.quadraticCurveTo(5.5 * f, hy + 16 * f, 5.5 * f, hy + 10.5 * f);
  ctx.stroke();
  /* くち：ひだりみぎ つりあいの とれた にっこり */
  const gp = (a >= 0) ? 5 * f : 0;
  ctx.lineWidth = 3 * f;
  ctx.beginPath();
  ctx.moveTo(-16 * f, hy + 19 * f);
  ctx.quadraticCurveTo(0, hy + 30 * f + gp, 16 * f, hy + 19 * f);
  ctx.stroke();
}

const PURIKING_DESIGNS = {
  hand:  { label: 'いまの てがき',      tex: 'ink',   face: null },
  anime: { label: 'アニメ調',          tex: 'anime', face: pkFaceAnime, earIn: true },
  cute:  { label: 'キュート',          tex: 'jelly', face: pkFaceCute,  headMul: 1.10 },
  real:  { label: 'ほんものの ねこ',    tex: 'ink',   face: pkFaceReal,  earIn: true, stripe: true, tail: true },
...
function drawPurikingStyled(ctx, s, P, T, D) {
  D = D || PURIKING_DESIGNS[PURIKING_DESIGN_PICK];
  T = T || PURIKING_TEX[D.tex || PURIKING_TEX_PICK];
  const a = s.atk;
  const step = Math.sin(s.t * 7) * (s.moving ? 1 : 0);
  ...
  const INK    = T.ink;
  const LW     = T.lw;
  const hy = P.hy, hr = P.hr * (D.headMul || 1), f = P.face * (D.headMul || 1);
  ...
  const fw = P.bw * 0.95;
  for (let i = 0; i < 3; i++) {
    const bx = -fw + i * fw + step * (i - 1) * 1.1;
    ctx.beginPath();
    ctx.moveTo(bx - 5, P.footTop);
    ctx.lineTo(bx - 2.5, -1);
    ctx.lineTo(bx,       P.footTop * 0.62);
    ctx.lineTo(bx + 2.5, -1);
    ctx.lineTo(bx + 5,   P.footTop);
    ctx.stroke();
  }
  ...
  /* --- しっぽ（ほんものの ねこ）は からだの うしろに --- */
  if (D.tail) {
    ctx.strokeStyle = '#d9862c'; ctx.lineWidth = 6.5; ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(-P.bw + 2, bcy + 4);
    ctx.quadraticCurveTo(-P.bw - 20, bcy + Math.sin(s.t * 3) * 5, -P.bw - 16, bcy - 22);
    ctx.stroke();
    ctx.strokeStyle = T.ink; ctx.lineWidth = 1.8;
    ctx.beginPath();
    ctx.moveTo(-P.bw + 2, bcy + 4);
    ctx.quadraticCurveTo(-P.bw - 20, bcy + Math.sin(s.t * 3) * 5, -P.bw - 16, bcy - 22);
    ctx.stroke();
  ...
  /* おなかの しろい ところ（ほんものの ねこ）*/
  if (D.stripe) {
    ctx.save(); bodyPath(); ctx.clip();
    ctx.fillStyle = 'rgba(255,248,230,.75)';
    ctx.beginPath(); ctx.ellipse(0, bcy + bh * 0.18, P.bw * 0.5, bh * 0.36, 0, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  ...
  for (const d of [-1, 1]) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(ex - d * 2, ey);
      ctx.quadraticCurveTo(ex + d * Math.cos(th) * len * 0.6, ey + Math.sin(th) * len * 0.5,
  ...
  for (const d of [-1, 1]) {
    ...
    /* みみの なかの ピンク */
    if (D.earIn) {
      ctx.save(); earPath(); ctx.clip();
      ctx.fillStyle = 'rgba(232,140,130,.8)';
      ctx.beginPath();
      ctx.moveTo(d * hr * 0.80, hy - hr * 0.44);
      ctx.lineTo(d * hr * 0.74, hy - hr * 0.44 - P.earLen * 0.72);
      ctx.lineTo(d * hr * 0.34, hy - hr * 0.80);
      ctx.closePath(); ctx.fill();
      ctx.restore();
    }
  }
  ...
  /* ふしぎな いきものの オーラ */
  if (D.aura) {
    const ag = ctx.createRadialGradient(0, hy, hr * 0.9, 0, hy, hr * 1.9);
    ag.addColorStop(0, 'rgba(180,230,255,.32)'); ag.addColorStop(1, 'rgba(180,230,255,0)');
    ctx.fillStyle = ag;
    ctx.beginPath(); ctx.arc(0, hy, hr * 1.9, 0, Math.PI * 2); ctx.fill();
  }
  purikingPaint(ctx, () => { ctx.beginPath(); ctx.arc(0, hy, hr, 0, Math.PI * 2); }, 0, hy, hr, T, s);
  /* ねこの しまもよう */
  if (D.stripe) {
    ctx.save(); ctx.beginPath(); ctx.arc(0, hy, hr, 0, Math.PI * 2); ctx.clip();
    ctx.strokeStyle = 'rgba(150,85,20,.55)'; ctx.lineWidth = 3.4; ctx.lineCap = 'round';
    for (let i = -1; i <= 1; i++) {
      ctx.beginPath();
      ctx.moveTo(i * 8 - 3, hy - hr + 1);
      ctx.lineTo(i * 8 + 1, hy - hr * 0.42);
      ctx.stroke();
    }
    for (const d of [-1, 1]) for (let i = 0; i < 2; i++) {
      ctx.beginPath();
      ctx.moveTo(d * (hr * 0.62), hy - 6 + i * 9);
      ctx.lineTo(d * (hr * 0.98), hy - 9 + i * 9);
      ctx.stroke();
    }
    ctx.restore();
  }

  /* --- かお。デザインが きまって いれば そちらを つかう --- */
  if (D.face) {
    D.face(ctx, hy, hr, f, a, INK);
    if (a >= 0) {
      ctx.strokeStyle = 'rgba(255,214,120,' + (0.75 * (1 - a)).toFixed(2) + ')';
      ctx.lineWidth = 4;
      ctx.beginPath(); ctx.arc(P.armX + 8, P.armY, 14 + a * 28, -0.9, 0.9); ctx.stroke();
    }
    ctx.restore();
    return;
  }
  ...
  for (const d of [-1, 1]) {
    ...
    ctx.quadraticCurveTo(d * 12.5 * f, hy - 7 * f, d * 10.5 * f, hy + 1 * f);
    ctx.stroke();
  }
  ...
  ctx.quadraticCurveTo(6 * f, hy + 18 * f, 6.5 * f, hy + 12 * f);
  ctx.quadraticCurveTo(7.5 * f, hy + 16 * f, 9.5 * f, hy + 18 * f);
  ctx.stroke();
  ...
  ctx.quadraticCurveTo(22 * f, hy + 27 * f + gape, 36 * f, hy + 11 * f);
  ctx.stroke();
...
/* --- 3大王 その1「ケダマール」（ぞくせい：パワー）---
     はいいろの おおきな けだま。まわりじゅうから ふとい けが とびだして いて、
     まんなかに 「3」の しるし。みぎに くろい めが 1つと、きばの わらい。 */
function drawKedamaru(ctx, st) {
  const t = (st && st.t) || 0;
  const atk = (st && st.atk !== undefined) ? st.atk : -1;
  const puff = (atk >= 0 && atk < 0.35) ? 1.10 : 1;     // こうげきの ときに ふくらむ
  const R = 62 * puff;
  const cy = -66;
  const BODY = '#c6c4bd', INK = '#1b1b1b';
  ...
  for (let i = 0; i < 18; i++) {
    const a = (i / 18) * Math.PI * 2 + Math.sin(t * 1.4 + i) * 0.05;
    const r0 = R * 0.92, r1 = R * (1.22 + ((i * 7) % 5) * 0.07);
    const bend = 0.22 * Math.sin(i * 2.1);
    ctx.beginPath();
    ctx.moveTo(Math.cos(a) * r0, cy + Math.sin(a) * r0 * 0.94);
    ctx.quadraticCurveTo(
      Math.cos(a + bend) * (r0 + r1) / 2, cy + Math.sin(a + bend) * (r0 + r1) / 2 * 0.94,
      Math.cos(a + bend * 1.6) * r1, cy + Math.sin(a + bend * 1.6) * r1 * 0.94);
    ctx.stroke();
  }

  /* からだ（まるい けだま）*/
  ctx.fillStyle = BODY; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.ellipse(0, cy, R, R * 0.94, 0, 0, Math.PI * 2);
  ctx.fill(); ctx.stroke();

  /* うちがわの けなみ（うずまく ながい け）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 3.4;
  for (let i = 0; i < 9; i++) {
    const a = (i / 9) * Math.PI * 2 + 0.3 + Math.sin(t * 1.2 + i) * 0.04;
    ctx.beginPath();
    ctx.moveTo(Math.cos(a) * R * 0.30, cy + Math.sin(a) * R * 0.28);
    ctx.quadraticCurveTo(
      Math.cos(a + 0.35) * R * 0.60, cy + Math.sin(a + 0.35) * R * 0.56,
      Math.cos(a + 0.10) * R * 0.86, cy + Math.sin(a + 0.10) * R * 0.82);
    ctx.stroke();
  }
  ...
  ctx.beginPath();
  ctx.moveTo(-20, cy - 22);
  ctx.quadraticCurveTo(-26, cy + 2, -14, cy + 20);
  ctx.quadraticCurveTo(6, cy + 28, 18, cy + 8);
  ctx.quadraticCurveTo(24, cy - 14, 6, cy - 24);
  ctx.quadraticCurveTo(-10, cy - 28, -20, cy - 22);
  ctx.stroke();
  ctx.fillStyle = INK;
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = '900 28px system-ui, sans-serif';
  ctx.fillText('3', 0, cy - 1);
  ctx.restore();

  /* め（みぎに 1つ・くろい まる）*/
  ctx.fillStyle = '#3a2420';
  ctx.beginPath(); ctx.arc(R * 0.62, cy - R * 0.22, 11, 0, Math.PI * 2); ctx.fill();

  /* きばの わらい（めの したに ニヤリ）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(R * 0.28, cy + R * 0.12);
  ctx.quadraticCurveTo(R * 0.56, cy + R * 0.28, R * 0.86, cy + R * 0.06);
  ctx.stroke();
  ctx.fillStyle = '#f6f3ea';
  for (const k of [0, 1, 2]) {
    const bx = R * (0.36 + k * 0.19), by = cy + R * (0.20 - k * 0.03);
    ctx.beginPath();
    ctx.moveTo(bx - 7, by);
    ctx.lineTo(bx, by + 19);
    ctx.lineTo(bx + 7, by);
...
/* --- 3大王 その2「ガオウドウ」（ぞくせい：むぞくせい）---
     オレンジの しかくい どう（からだ）に おおきな きばの くち。
     みぎうえに もう 1つ、おこった かおの しかくい あたま。
     みぎの ふちには とげ。したに ほそい あし 2ほん。            */
function drawGaoudou(ctx, st) {
  const t = (st && st.t) || 0;
  const atk = (st && st.atk !== undefined) ? st.atk : -1;
  const bite = (atk >= 0 && atk < 0.30) ? 1 : 0;        // かみつく しゅんかん
  ...
  /* あし 2ほん */
  ctx.fillStyle = OR2;
  ctx.beginPath(); ctx.moveTo(-30, -34); ctx.lineTo(-22, 0); ctx.lineTo(-13, -34); ctx.closePath();
  ctx.fill(); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(13, -34); ctx.lineTo(22, 0); ctx.lineTo(30, -34); ctx.closePath();
  ctx.fill(); ctx.stroke();

  /* みぎの とげ */
  ctx.fillStyle = OR2;
  for (const ty of [-92, -62]) {
    ctx.beginPath();
    ctx.moveTo(46, ty + 12); ctx.lineTo(70, ty); ctx.lineTo(46, ty - 12);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }

  /* どう（しかくい からだ）*/
  ctx.fillStyle = OR;
  roundRect(ctx, -48, -140, 96, 108, 12);
  ctx.fill(); ctx.stroke();
  /* しま もよう */
  ctx.save();
  ctx.beginPath(); roundRect(ctx, -48, -140, 96, 108, 12); ctx.clip();
  ctx.strokeStyle = 'rgba(255,255,255,.18)'; ctx.lineWidth = 6;
  for (let i = -14; i < 14; i++) {
    ctx.beginPath(); ctx.moveTo(-60 + i * 12, -32); ctx.lineTo(-24 + i * 12, -146); ctx.stroke();
  }
  ctx.restore();
  ...
  ctx.beginPath();
  ctx.moveTo(-40, -118);
  ctx.quadraticCurveTo(-6, -104, 34, -122);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(-40, -58 - bite * 6);
  ctx.quadraticCurveTo(-4, -44 - bite * 8, 36, -62 - bite * 6);
  ctx.stroke();
  ctx.fillStyle = '#f8f4e8';
  /* うえの きば（したむき）*/
  for (const k of [0, 1, 2]) {
    const bx = -30 + k * 28, by = -116 + k * 2;
    ctx.beginPath();
    ctx.moveTo(bx - 11, by); ctx.lineTo(bx, by + 34); ctx.lineTo(bx + 11, by);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  /* したの きば（うわむき）*/
  for (const k of [0, 1]) {
    const bx = -14 + k * 30, by = -52 - bite * 7;
    ctx.beginPath();
    ctx.moveTo(bx - 12, by); ctx.lineTo(bx, by - 30); ctx.lineTo(bx + 12, by);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }

  /* みぎうえの あたま（しかく・おこった かお）*/
  ctx.save();
  ctx.translate(58, -176 + Math.sin(t * 2) * 2);
  ctx.fillStyle = OR;
  roundRect(ctx, -44, -46, 88, 92, 6);
  ctx.fill(); ctx.stroke();
  /* め（うちがわに つりあがった おこりがお）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 5; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-32, -34); ctx.lineTo(-6, -16);       // ひだり ＼
  ctx.moveTo(32, -34);  ctx.lineTo(8, -16);        // みぎ ／
  ctx.stroke();
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(-20, -14, 7, 9, -0.3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(20, -14, 7, 9, 0.3, 0, Math.PI * 2); ctx.fill();
  /* きばの くち */
  ctx.fillStyle = '#f8f4e8'; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(-28, 4); ctx.lineTo(30, 4); ctx.lineTo(18, 34); ctx.lineTo(-16, 34);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath();
  ctx.moveTo(-24, 14); ctx.lineTo(-12, 24); ctx.lineTo(-2, 12);
  ctx.lineTo(10, 24); ctx.lineTo(22, 12);
  ctx.stroke();
  ctx.restore();
...
/* --- 3大王 その3「チューチュー」（ぞくせい：けもの）---
     みどりの おおきな かの からだ。くろい ふとい ふちどり。
     みぎに ながい くちばり（こうふん）、うしろに はね、したに ほそい あし。 */
function drawChuchu(ctx, st) {
  const t = (st && st.t) || 0;
  const atk = (st && st.atk !== undefined) ? st.atk : -1;
  const suck = (atk >= 0 && atk < 0.4) ? 1 : 0;
  const GR = '#6fd7a8', GR2 = '#4bb98a', INK = '#111111';
  ...
  for (const d of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      const sx = d * (14 + i * 5), sy = -30 + i * 6;
      ctx.beginPath();
      ctx.moveTo(sx, sy);
      ctx.lineTo(sx + d * (16 + i * 6), sy - 12 - i * 4);
      ctx.lineTo(sx + d * (22 + i * 10), sy + 14 + i * 3);
      ctx.stroke();
    }
  }
  ...
  ctx.save();
  ctx.translate(-20, -76);
  ctx.rotate(0.10 + Math.sin(t * 9) * 0.10);
  ctx.strokeStyle = INK; ctx.lineWidth = 3.4; ctx.fillStyle = 'rgba(255,255,255,.30)';
  for (const k of [0, 1]) {
    ctx.save();
    ctx.rotate(k * 0.40);
    ctx.beginPath();
    ctx.ellipse(-40, 10, 44, 15, 0.22, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
    ctx.restore();
  }
  ctx.restore();
  ...
  /* どう（したの ちいさな からだ）*/
  ctx.fillStyle = GR2; ctx.strokeStyle = INK; ctx.lineWidth = 4;
  roundRect(ctx, -22, -78, 44, 52, 12);
  ctx.fill(); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.moveTo(0, -38); ctx.lineTo(0, -62);
  ctx.moveTo(0, -62); ctx.lineTo(-7, -54); ctx.moveTo(0, -62); ctx.lineTo(7, -54);
  ctx.stroke();

  /* あたま／おなか（おおきな たまご）*/
  ctx.fillStyle = GR; ctx.strokeStyle = INK; ctx.lineWidth = 9;
  ctx.beginPath(); ctx.ellipse(0, -136, 46, 60, 0, 0, Math.PI * 2);
  ctx.fill(); ctx.stroke();
  /* すった ちが たまる（こうげきの とき すこし あかく）*/
  if (suck) {
    ctx.fillStyle = 'rgba(229,57,53,.45)';
    ctx.beginPath(); ctx.ellipse(0, -118, 28, 32, 0, 0, Math.PI * 2); ctx.fill();
  }
  /* め（ほそい 1ぼん）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 4;
  ctx.beginPath();
  ctx.moveTo(14, -166); ctx.quadraticCurveTo(20, -150, 16, -134);
  ctx.stroke();

  /* くちばり（みぎへ ながく）*/
  ctx.fillStyle = GR2; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  const tip = 112 + suck * 12;
  ctx.beginPath();
  ctx.moveTo(40, -142);
  ctx.lineTo(tip, -128);
  ctx.lineTo(40, -120);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 3.2;
  ctx.beginPath(); ctx.moveTo(tip - 14, -128); ctx.lineTo(tip + 6, -126); ctx.stroke();

  ctx.restore();
}

/* =====================================================================
   ★15しょう「虫に しはいされた まち」の てき
   ===================================================================== */

/* --- トゲハヤさん（タツマキ）---
     もとの トゲハヤさんの あたまの トゲが、おおきな たつまきに なって いる。
     ごうのすけくんの えの とおり、はいいろの ろうとの まわりに
     くろい ぐるぐるの かぜ。                                           */
function drawTogehayaT(ctx, s) {
  const t = (s && s.t) || 0;
  ...
  for (let k = 0; k < 3; k++) {
    ...
    for (let i = 0; i <= 34; i++) {
      const f = i / 34;
      const a = f * Math.PI * 4.4 + k * 2.0 + t * 1.6;
      const rad = 10 + f * 30;
      const x = 3 + Math.cos(a) * rad;
      const y = -78 - f * 34 + Math.sin(a) * rad * 0.30;
      if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    ctx.stroke();
  }
  ...
  /* たつまきの ろうと（はいいろ）*/
  ctx.save();
  ctx.fillStyle = 'rgba(120,124,132,.85)';
  ctx.strokeStyle = '#5b6068'; ctx.lineWidth = 1.6;
  ctx.beginPath();
  ctx.moveTo(-1, -50);
  ctx.lineTo(-13, -104);
  ctx.lineTo(19, -104);
  ...
  /* もとの トゲハヤさん（トゲの ぶんは たつまきが かわりに なる）*/
  const step = Math.sin(t * 8) * (s && s.moving ? 1 : 0);
  ctx.save();
  ctx.fillStyle = '#455a64';
  roundRect(ctx, -14, -12 + step, 10, 12, 2); ctx.fill();
  roundRect(ctx, 5, -12 - step, 10, 12, 2); ctx.fill();
  ...
  ellipse(ctx, 6, -36, 3.4, 3.4); ctx.fill();
  /* くちは「ぎざぎざの こまった かお」*/
  ctx.strokeStyle = '#ffab91'; ctx.lineWidth = 1.8; ctx.lineJoin = 'round';
  ctx.beginPath();
  ctx.moveTo(-8, -29);
  ctx.lineTo(-3, -32); ctx.lineTo(1, -29); ctx.lineTo(5, -32); ctx.lineTo(9, -29);
  ctx.stroke();
...
/* --- ハッチー ---
     みずいろの「ちゅうしゃき」の むし。まるい あたまに たてぼう 2ほんの め。
     みぎに めもりの ついた ながい はり、うしろに おおきな さんかくの はね。
     そらを とんで います。                                             */
function drawHatchie(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const jab = (atk >= 0 && atk < 0.35) ? 8 : 0;      // さすとき まえに でる
  const BLUE = '#5bb6e8', BLUE2 = '#3f9fd6', PINK = '#e05a86';

  ctx.save();
  ctx.translate(jab, Math.sin(t * 5) * 2);
  ...
  ctx.fillStyle = '#ffffff';
  for (let i = 0; i < 7; i++) {
    ctx.fillRect(16 + i * 6.4, -43 + (i % 2) * 3, 2.2, (i % 2) ? 5 : 8);
  }
  /* ピンクの しるし */
  ctx.fillStyle = PINK;
  ctx.fillRect(13, -45, 3, 12);
  ctx.fillRect(34, -45, 3, 12);
  /* おしりの つば（ピンク）*/
  ctx.fillRect(62, -50, 5, 24);

  /* あたま（まるい）*/
  ctx.fillStyle = BLUE;
  ctx.beginPath(); ctx.arc(0, -44, 22, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = BLUE2;
  ctx.beginPath(); ctx.arc(0, -44, 22, 0.25 * Math.PI, 0.95 * Math.PI); ctx.fill();

  /* め（たてぼう 2ほん）*/
  ctx.strokeStyle = '#111111'; ctx.lineWidth = 3.2; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-6, -58); ctx.lineTo(-6, -48);
  ctx.moveTo(4, -58);  ctx.lineTo(4, -48);
  ctx.stroke();
...
/* --- カマジロウ ---
     みどりの カマキリ。さんかくの あたまに おおきな め、
     りょうての かまを ふりおろして あばれる まえ えいの てき。          */
function drawKamajirou(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const swing = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;   // かまを ふる
  const step = Math.sin(t * 7) * (s && s.moving ? 1.5 : 0);
  const GR = '#7cc36b', GR2 = '#4e9a45', INK = '#23401f';
  ...
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.moveTo(-6 + d * 4, -26);
    ctx.lineTo(-20 + d * 6, -16 + step * d);
    ctx.lineTo(-10 + d * 10, 0);
    ctx.stroke();
  }
  ...
  ctx.fillStyle = GR2;
  ctx.beginPath();
  ctx.ellipse(-20, -34, 24, 11, -0.28, 0, Math.PI * 2);
  ctx.fill(); ctx.lineWidth = 2.2; ctx.stroke();

  /* むね */
  ctx.fillStyle = GR;
  roundRect(ctx, -10, -60, 20, 30, 8); ctx.fill(); ctx.stroke();

  /* かま（りょうて）*/
  for (const d of [0, 1]) {
    ctx.save();
    ctx.translate(8, -54 + d * 7);
    ctx.rotate(-0.30 + swing * 1.05 + d * 0.16);
    ctx.fillStyle = GR;
    roundRect(ctx, 0, -4, 22, 8, 4); ctx.fill(); ctx.stroke();   // うでの もと
    ctx.translate(22, 0);
    ctx.rotate(-0.85 + swing * 0.5);
    ctx.fillStyle = GR2;
    ctx.beginPath();
    ctx.moveTo(0, -4);
    ctx.quadraticCurveTo(26, -16, 30, 2);
    ctx.quadraticCurveTo(18, -2, 0, 5);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    /* かまの ぎざぎざ */
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    for (let i = 0; i < 4; i++) { ctx.moveTo(5 + i * 6, -1); ctx.lineTo(8 + i * 6, -7); }
    ctx.stroke();
    ctx.restore();
  }
  ...
  /* おおきな め */
  ctx.fillStyle = '#fff59d';
  ctx.beginPath(); ctx.ellipse(-5, -1, 5.5, 6.5, -0.2, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.beginPath(); ctx.ellipse(8, 0, 5.0, 6.0, 0.2, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#23401f';
  ctx.beginPath(); ctx.arc(-4, 0, 2.2, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(9, 1, 2.0, 0, Math.PI * 2); ctx.fill();
  /* しょっかく */
  ctx.lineWidth = 1.8;
  ctx.beginPath();
  ctx.moveTo(-2, -10); ctx.quadraticCurveTo(6, -24, 18, -26);
  ctx.moveTo(2, -10);  ctx.quadraticCurveTo(12, -20, 24, -18);
  ctx.stroke();
  ctx.restore();
...
function drawHaneneko(ctx, s) {
  ...
  const WING = '#7a6f68', WING2 = '#5d544e';

  ctx.save();
  ctx.lineJoin = 'round'; ctx.lineCap = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;

  /* はね（コウモリの はね・2まい。おくの 1まいを こい いろで）*/
  for (const [w, col, dx, dr] of [[0.88, WING2, -12, -0.12], [1.00, WING, 0, 0]]) {
    ctx.save();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 6);                       // つけね
    ctx.lineTo(-4, -20);                    // まえの ほね
    ctx.lineTo(4, -44);                     // いちばん うえの つめ
    /* うしろがわは ほねの あいだが たれさがる（コウモリらしい かたち）*/
    ctx.quadraticCurveTo(12, -34, 20, -40);
    ctx.quadraticCurveTo(26, -28, 34, -32);
    ctx.quadraticCurveTo(36, -18, 42, -20);
    ctx.quadraticCurveTo(30, -4, 12, 2);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    /* ゆびの ほね */
    ctx.lineWidth = 1.8 / w;
    ctx.beginPath();
    ctx.moveTo(1, 2); ctx.lineTo(4, -44);
    ctx.moveTo(1, 2); ctx.lineTo(20, -40);
    ctx.moveTo(1, 2); ctx.lineTo(34, -32);
    ctx.moveTo(1, 2); ctx.lineTo(42, -20);
    ctx.stroke();
    ctx.restore();
  }

  /* しっぽ（うしろで くるんと うえに まく）*/
  ctx.strokeStyle = ORA; ctx.lineWidth = 9; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(-34, -44);
  ctx.bezierCurveTo(-56, -52, -58, -86, -34, -88);
  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.0;
  ctx.beginPath();
  ctx.moveTo(-34, -44);
  ...
  /* あし 4ほん（さきは しろい くつした）*/
  for (const [x, d] of [[-24, 1], [-14, -1], [20, -1], [30, 1]]) {
    ctx.strokeStyle = CRE; ctx.lineWidth = 8;
    ctx.beginPath();
    ctx.moveTo(x, -28);
    ctx.lineTo(x + step * d, -4);
    ctx.stroke();
    ctx.fillStyle = GRY;
    ctx.beginPath(); ctx.ellipse(x + step * d, -3, 6, 4, 0, 0, Math.PI * 2); ctx.fill();
  }

  /* どう（うえが ちゃとら・したが しろ）*/
  ctx.fillStyle = ORA; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath();
  ctx.ellipse(-2, -46, 38, 19, 0, 0, Math.PI * 2);
  ctx.fill(); ctx.stroke();
  ctx.save();
  ctx.beginPath(); ctx.ellipse(-2, -46, 38, 19, 0, 0, Math.PI * 2); ctx.clip();
  ctx.fillStyle = CRE;
  ctx.fillRect(-42, -44, 82, 24);
  /* とらの しま */
  ctx.strokeStyle = ORA2; ctx.lineWidth = 3.4;
  for (const x of [-22, -10, 2, 14, 26]) {
    ctx.beginPath(); ctx.moveTo(x, -64); ctx.lineTo(x - 5, -46); ctx.stroke();
  }
  ctx.restore();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.ellipse(-2, -46, 38, 19, 0, 0, Math.PI * 2); ctx.stroke();

  /* くび */
  ctx.fillStyle = CRE; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.ellipse(34, -54, 13, 12, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

  /* みみ 2つ（あたまの まえに かいて、つけねを あたまで かくす）*/
  for (const [ex, ey, lean] of [[34, -78, -0.20], [50, -80, 0.10]]) {
    ctx.fillStyle = CRE;
    ctx.save(); ctx.translate(ex, ey); ctx.rotate(lean);
    ctx.beginPath();
    ctx.moveTo(-7, 8); ctx.lineTo(0, -14); ctx.lineTo(7, 8);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#e8b9a6';                 // みみの なか
    ctx.beginPath();
    ctx.moveTo(-3.4, 6); ctx.lineTo(0, -7); ctx.lineTo(3.4, 6);
    ctx.closePath(); ctx.fill();
    ctx.restore();
  }
  ...
  /* あたま */
  ctx.fillStyle = CRE; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.ellipse(43, -70, 18, 16, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* あたまの とらしま */
  ctx.save();
  ctx.beginPath(); ctx.ellipse(43, -70, 18, 16, 0, 0, Math.PI * 2); ctx.clip();
  ctx.strokeStyle = ORA2; ctx.lineWidth = 3;
  for (const x of [38, 45]) { ctx.beginPath(); ctx.moveTo(x, -88); ctx.lineTo(x - 2, -76); ctx.stroke(); }
  ctx.restore();

  /* はなさき（みぎに すこし つきでる）*/
  ctx.fillStyle = CRE; ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.ellipse(56, -65, 8, 6.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

  /* め・はな・くち */
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(48, -73, 3.4, 4.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#c4756b';
  ctx.beginPath();
  ctx.moveTo(59, -68); ctx.lineTo(63, -66); ctx.lineTo(59, -64); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = INK; ctx.lineWidth = 1.8;
  ctx.beginPath();
  ctx.moveTo(60, -64); ctx.quadraticCurveTo(57, -60, 53, -62);
  ctx.stroke();
  /* ひげ */
  ctx.lineWidth = 1.4;
  for (const dy of [-3, 1]) {
...
/* --- キョーダイ（だい17しょう）---
     おしばいの おめんを かぶった ふたりぐみ。★1たいの てき です★。
     あかい ほうが わらいめん、あおい ほうが なきめん。
     ★たいりょくが へると なきめんが ないて、ふたりとも あばれだす
       （enrage）。                                                   */
function drawKyoudai(ctx, s) {
  const t = (s && s.t) || 0;
  const mad = !!(s && s.enraged);
  const sway = Math.sin(t * 4) * 2.2;
  const INK = '#23303a', MASK = '#6fc0e4', MASK2 = '#8ed3f0';
  const RED = '#c0392f', BLU = '#4f9fd0';

  /* おめん 1まい（cx, cy が おめんの まんなか）*/
  function mask(cx, cy, sc, smile) {
    ctx.save();
    ctx.translate(cx, cy); ctx.scale(sc, sc);
    /* たまごを さかさに した ような かたち */
    const g = ctx.createLinearGradient(0, -30, 0, 30);
    g.addColorStop(0, MASK2); g.addColorStop(1, MASK);
    ctx.fillStyle = g; ctx.strokeStyle = INK; ctx.lineWidth = 2.6 / sc;
    ctx.beginPath();
    ctx.moveTo(-24, -22);
    ctx.quadraticCurveTo(-24, -34, -8, -33);
    ctx.quadraticCurveTo(0, -32, 8, -33);
    ctx.quadraticCurveTo(24, -34, 24, -22);
    ctx.quadraticCurveTo(24, 20, 0, 33);
    ctx.quadraticCurveTo(-24, 20, -24, -22);
    ctx.closePath(); ctx.fill(); ctx.stroke();

    ctx.fillStyle = '#ffffff'; ctx.lineWidth = 1.6 / sc;
    /* まゆ */
    for (const d of [-1, 1]) {
      ctx.beginPath();
      if (smile) {
        ctx.moveTo(d * 4, -20); ctx.quadraticCurveTo(d * 11, -26, d * 18, -18);
        ctx.quadraticCurveTo(d * 11, -22, d * 4, -17);
      } else {
        ctx.moveTo(d * 4, -24); ctx.quadraticCurveTo(d * 12, -20, d * 17, -10);
        ctx.quadraticCurveTo(d * 12, -16, d * 4, -21);
      }
      ctx.closePath(); ctx.fill(); ctx.stroke();
    }
    /* め */
    for (const d of [-1, 1]) {
      ctx.beginPath();
      if (smile) {
        ctx.moveTo(d * 4, -8); ctx.quadraticCurveTo(d * 11, -14, d * 17, -7);
        ctx.quadraticCurveTo(d * 11, -10, d * 4, -6);
      } else {
        ctx.moveTo(d * 4, -11); ctx.quadraticCurveTo(d * 11, -4, d * 17, -9);
        ctx.quadraticCurveTo(d * 11, -8, d * 4, -9);
      }
      ctx.closePath(); ctx.fill(); ctx.stroke();
    }
    /* くち */
    ...
    if (smile) {
      ctx.moveTo(-15, 6);
      ctx.quadraticCurveTo(0, 26, 15, 6);
      ctx.quadraticCurveTo(0, 14, -15, 6);
    } else {
      ctx.moveTo(-13, 20);
      ctx.quadraticCurveTo(0, 4, 13, 20);
      ctx.quadraticCurveTo(0, 12, -13, 20);
    }
    ctx.closePath(); ctx.fill(); ctx.stroke();
    /* ほおの せん */
    ...
    for (const d of [-1, 1]) {
      ctx.beginPath();
      ctx.moveTo(d * 20, -4);
      ctx.quadraticCurveTo(d * 17, 10, d * 11, 18);
      ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 3 / sc; ctx.stroke();
      ctx.strokeStyle = INK; ctx.lineWidth = 1.2 / sc; ctx.stroke();
    }
    ctx.restore();
  }
  ...
  /* ---- あおい なきめんの ほう（うしろ・ひだり）---- */
  ctx.save();
  ctx.translate(-32 - sway * 0.4, 0);
  ctx.fillStyle = BLU; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  /* どう */
  ctx.fillRect(-15, -52, 30, 32);
  /* うでは よこに たらす */
  ctx.fillRect(-29, -48, 15, 10);
  ctx.fillRect(14, -48, 15, 10);
  /* あし */
  ctx.beginPath();
  ctx.moveTo(-13, -22); ctx.lineTo(-4, -22); ctx.lineTo(-6, 0); ctx.lineTo(-13, -4);
  ctx.closePath(); ctx.fill();
  ctx.beginPath();
  ctx.moveTo(4, -22); ctx.lineTo(13, -22); ctx.lineTo(13, -4); ctx.lineTo(6, 0);
  ctx.closePath(); ctx.fill();
  mask(0, -68, 0.86, false);
  /* ★ないて いる（ピンチの とき）*/
  if (mad) {
    ctx.fillStyle = '#4fc3f7';
    for (const d of [-1, 1]) {
      const dy = ((t * 60) % 22);
      ctx.beginPath();
      ctx.ellipse(d * 10, -70 + dy, 2.6, 4.2, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.restore();
  ...
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.moveTo(d * 12, -54);
    ctx.lineTo(d * 30, -76 - up);
    ctx.lineTo(d * 22, -80 - up);
    ctx.lineTo(d * 8, -60);
    ctx.closePath(); ctx.fill();
  }
  mask(0, -74, 0.92, true);
...
/* ==========================================================================
   だい18しょう ★とくせつ「地底の国」★ の キャラたち（1〜3面）
   ========================================================================== */

/* --- トリエル（1面ボス）---
     いせきに すむ やぎの おかあさん。むらさきの ローブ。
     ほのおの まほうを つかうが、どこか てかげんして いる。            */
function drawToriel(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#2b2233', ROBE = '#6b4f96', ROBE2 = '#543d78';
  const FUR = '#f4efe4', FUR2 = '#ded6c6';
  ...
  ctx.moveTo(-24, -96);
  ctx.lineTo(24, -96);
  ctx.quadraticCurveTo(38, -40, 44, 0);
  ctx.lineTo(-44, 0);
  ctx.quadraticCurveTo(-38, -40, -24, -96);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* すその かげ */
  ctx.fillStyle = ROBE2;
  ctx.beginPath();
  ctx.moveTo(-42, -16); ctx.lineTo(42, -16); ctx.lineTo(44, 0); ctx.lineTo(-44, 0);
  ctx.closePath(); ctx.fill();

  /* デルタルーンの もんしょう（しろい さんかくと わっか） */
  ctx.fillStyle = '#f0ecdf';
  ctx.beginPath(); ctx.arc(2, -62, 11, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = ROBE;
  ctx.beginPath(); ctx.arc(2, -62, 7, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#f0ecdf';
  ctx.beginPath();
  ctx.moveTo(2, -80); ctx.lineTo(13, -66); ctx.lineTo(-9, -66); ctx.closePath(); ctx.fill();

  /* うで（みぎては ほのおを だす） */
  ctx.strokeStyle = INK; ctx.lineWidth = 3; ctx.fillStyle = FUR;
  ctx.beginPath();
  ctx.moveTo(22, -92);
  ctx.quadraticCurveTo(44, -86 - cast * 10, 50 + cast * 6, -66 - cast * 14);
  ctx.lineWidth = 11; ctx.strokeStyle = ROBE; ctx.stroke();
  ctx.lineWidth = 3; ctx.strokeStyle = INK;
  ctx.beginPath(); ctx.arc(51 + cast * 6, -64 - cast * 14, 7, 0, Math.PI * 2);
  ctx.fillStyle = FUR; ctx.fill(); ctx.stroke();
  /* ほのおの たま */
  if (cast > 0.15) {
    ctx.save();
    ctx.globalAlpha = cast;
    const fx = 62 + cast * 10, fy = -66 - cast * 16, r = 8 + cast * 6;
    ctx.fillStyle = '#ff7043';
    ctx.beginPath();
    ctx.moveTo(fx, fy - r * 1.5);
    ctx.quadraticCurveTo(fx + r, fy - r * 0.2, fx, fy + r);
    ctx.quadraticCurveTo(fx - r, fy - r * 0.2, fx, fy - r * 1.5);
    ctx.fill();
    ctx.fillStyle = '#ffe082';
    ctx.beginPath(); ctx.arc(fx, fy, r * 0.42, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }

  /* たれみみ（あたまより さきに かいて、うしろに まわす） */
  ctx.fillStyle = FUR2; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  for (const [ex, lean] of [[-22, -0.30], [20, 0.26]]) {
    ctx.save(); ctx.translate(ex, -122); ctx.rotate(lean);
    ctx.beginPath(); ctx.ellipse(0, 20, 9, 24, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.restore();
  }
  /* つの（ちいさく 2ほん） */
  ...
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.moveTo(-2 + d * 13, -142);
    ctx.quadraticCurveTo(-2 + d * 20, -158, -2 + d * 9, -152);
    ctx.quadraticCurveTo(-2 + d * 6, -146, -2 + d * 13, -142);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }

  /* あたま（やぎ。みぎに はなが のびる） */
  ctx.fillStyle = FUR; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.ellipse(-2, -124, 27, 25, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* ながい はなづら */
  ctx.beginPath();
  ctx.moveTo(14, -136);
  ctx.quadraticCurveTo(44, -132, 42, -114);
  ctx.quadraticCurveTo(28, -106, 12, -112);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* はな */
  ctx.fillStyle = '#c9a0a8';
  ctx.beginPath(); ctx.ellipse(40, -122, 5, 4, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* め（やさしい たれめ） */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath(); ctx.arc(10, -130, 5.5, Math.PI * 0.05, Math.PI * 0.95); ctx.stroke();
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(10, -132, 3.4, 4, 0, 0, Math.PI * 2); ctx.fill();
  /* くち */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath();
  ctx.moveTo(24, -114); ctx.quadraticCurveTo(30, -109, 36, -113);
  ctx.stroke();
  /* ほっぺ */
  ctx.fillStyle = 'rgba(224,130,150,.40)';
  ctx.beginPath(); ctx.ellipse(6, -118, 8, 5, 0, 0, Math.PI * 2); ctx.fill();

  ctx.restore();
}

/* --- パピルス（2面ボス）---
     せの たかい ガイコツ。しろい「バトルボディ」と あかい マフラー。
     ★あおい ほねで あいての うごきを とめる。                        */
function drawPapyrus(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const pose = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#23201e', BONE = '#f6f3e8', ARMOR = '#eceadf';
  const RED = '#d2342c';
  ...
  /* あし（あかい ブーツ） */
  for (const d of [-1, 1]) {
    ctx.strokeStyle = BONE; ctx.lineWidth = 8;
    ctx.beginPath(); ctx.moveTo(d * 11, -58); ctx.lineTo(d * 13, -22); ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
    ctx.beginPath(); ctx.moveTo(d * 11, -58); ctx.lineTo(d * 13, -22); ctx.stroke();
    ctx.fillStyle = RED;
    roundRect(ctx, d * 13 - 11, -24, 24, 24, 6); ctx.fill(); ctx.stroke();
  }

  /* どう（バトルボディ） */
  ctx.fillStyle = ARMOR; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-21, -104);
  ctx.lineTo(21, -104);
  ctx.lineTo(25, -62);
  ...
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* むねの まるい かざり */
  ctx.fillStyle = '#f3c43f';
  ctx.beginPath(); ctx.arc(0, -88, 8, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* こしの あかい パンツ */
  ctx.fillStyle = RED;
  roundRect(ctx, -24, -68, 48, 14, 4); ctx.fill(); ctx.stroke();
  ...
  ctx.lineTo(-20, -98);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  const wav = Math.sin(t * 3) * 6;
  ctx.beginPath();
  ctx.moveTo(-18, -106);
  ctx.quadraticCurveTo(-44, -102 + wav, -58, -84 + wav);
  ctx.lineTo(-50, -74 + wav);
  ctx.quadraticCurveTo(-40, -92 + wav, -16, -96);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* うで（あかい てぶくろ）。こうげきで ほねを かかげる */
  ctx.strokeStyle = BONE; ctx.lineWidth = 7;
  ctx.beginPath();
  ctx.moveTo(19, -100);
  ctx.lineTo(36, -92 - pose * 22);
  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.moveTo(19, -100); ctx.lineTo(36, -92 - pose * 22);
  ctx.stroke();
  ctx.fillStyle = RED;
  ctx.beginPath(); ctx.arc(38, -91 - pose * 22, 8, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* ★あおい ほね */
  if (pose > 0.15) {
    ctx.save();
    ctx.globalAlpha = Math.min(1, pose * 1.4);
    ctx.translate(46, -100 - pose * 26); ctx.rotate(0.5);
    ctx.fillStyle = '#4fc3f7'; ctx.strokeStyle = '#e1f5fe'; ctx.lineWidth = 2;
    roundRect(ctx, -4, -20, 8, 40, 3); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#4fc3f7';
    for (const ey of [-20, 20]) {
      ctx.beginPath(); ctx.arc(-4, ey, 4, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(4, ey, 4, 0, Math.PI * 2); ctx.fill();
    }
    ctx.restore();
  }
  ...
  /* あたま（ガイコツ） */
  ctx.fillStyle = BONE; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-17, -118);
  ctx.quadraticCurveTo(-19, -146, 2, -148);
  ctx.quadraticCurveTo(23, -146, 21, -118);
  ctx.quadraticCurveTo(18, -108, 2, -108);
  ctx.quadraticCurveTo(-14, -108, -17, -118);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* めの あな（かたほうだけ まゆげが あがる） */
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(4, -131, 4.4, 5.6, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(16, -130, 4.0, 5.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(0, -141); ctx.lineTo(9, -139); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(12, -140); ctx.lineTo(21, -143); ctx.stroke();
  /* おおきな わらい */
  ctx.beginPath();
  ctx.moveTo(-6, -119); ctx.quadraticCurveTo(6, -110, 19, -119);
  ctx.stroke();
  for (const x of [0, 6, 12]) {
    ctx.beginPath(); ctx.moveTo(x, -117.5); ctx.lineTo(x, -113); ctx.stroke();
  }
...
/* --- アンダイン（3面ボス）---
     あおい さかなの せんし。あかい ポニーテール、ひだりめに がんたい、
     ぎんの よろい。やりを なげる。
     ★たいりょくが はんぶんに なると「アンダイン・ジ・アンダイング」に。 */
function drawUndyne(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const thr = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const mad = !!(s && s.enraged);
  const INK = '#1c2630', SKIN = '#4fa8d8', SKIN2 = '#3b8cbb';
  const ARM = mad ? '#3a3f4a' : '#c9d2da', HAIR = '#e2452f';
  ...
  ctx.beginPath();
  ctx.moveTo(-8, -118);
  ctx.quadraticCurveTo(-52, -112 + Math.sin(t * 3) * 5, -64, -58 + Math.sin(t * 3) * 8);
  ctx.lineTo(-44, -52);
  ctx.quadraticCurveTo(-40, -96, -6, -104);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* あし */
  for (const d of [-1, 1]) {
    ctx.strokeStyle = SKIN2; ctx.lineWidth = 11;
    ctx.beginPath(); ctx.moveTo(d * 12, -52); ctx.lineTo(d * 15, -6); ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.moveTo(d * 12, -52); ctx.lineTo(d * 15, -6); ctx.stroke();
    ctx.fillStyle = ARM;
    roundRect(ctx, d * 15 - 10, -10, 21, 11, 4); ctx.fill(); ctx.stroke();
  }

  /* どう（よろい） */
  ctx.fillStyle = ARM; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-22, -102);
  ctx.lineTo(22, -102);
  ctx.lineTo(26, -50);
  ...
  /* かたの プレート */
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.ellipse(d * 24, -98, 11, 9, 0, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }
  /* むねの すじ */
  ctx.strokeStyle = 'rgba(0,0,0,.28)'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(0, -100); ctx.lineTo(0, -54); ctx.stroke();

  /* うで と やり */
  ctx.strokeStyle = SKIN; ctx.lineWidth = 9;
  ctx.beginPath();
  ctx.moveTo(20, -96);
  ctx.lineTo(40 + thr * 14, -104 - thr * 10);
  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(20, -96); ctx.lineTo(40 + thr * 14, -104 - thr * 10); ctx.stroke();
  /* やり */
  ctx.save();
  ctx.translate(42 + thr * 14, -104 - thr * 10);
  ctx.rotate(-0.55 + thr * 0.5);
  /* え（こい いろ ＋ ふちどり。うすい いろだと はいけいに とける）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 9;
  ctx.beginPath(); ctx.moveTo(-30, 26); ctx.lineTo(26, -34); ctx.stroke();
  ctx.strokeStyle = mad ? '#29b6f6' : '#7d5a3a'; ctx.lineWidth = 5.5;
  ctx.beginPath(); ctx.moveTo(-30, 26); ctx.lineTo(26, -34); ctx.stroke();
  ctx.fillStyle = mad ? '#4fc3f7' : '#b0bec5'; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath();
  ctx.moveTo(38, -46); ctx.lineTo(22, -40); ctx.lineTo(28, -24);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.restore();

  /* あたま */
  ctx.fillStyle = SKIN; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.ellipse(2, -122, 22, 21, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* えらの ひれ（あたまの うしろに 2まい） */
  ctx.fillStyle = SKIN2;
  for (const dy of [-6, 6]) {
    ctx.beginPath();
    ctx.moveTo(-18, -124 + dy);
    ctx.lineTo(-38, -130 + dy * 1.6);
    ctx.lineTo(-34, -116 + dy * 1.2);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  /* まえがみ（あたまの うえを おおう） */
  ctx.fillStyle = HAIR;
  ctx.beginPath();
  ctx.moveTo(-22, -128);
  ctx.quadraticCurveTo(-4, -154, 24, -132);
  ctx.quadraticCurveTo(14, -138, 4, -134);
  ctx.quadraticCurveTo(-8, -138, -22, -128);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* みぎめ（きいろ／ピンチで あかく ひかる）*/
  ctx.fillStyle = mad ? '#ff5252' : '#ffe082';
  ctx.beginPath(); ctx.ellipse(15, -124, 7, 7.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.arc(17, -124, 3.2, 0, Math.PI * 2); ctx.fill();
  /* ★がんたい（ひだりめ）。ひもを あたまの うしろまで とおす */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath(); ctx.moveTo(-20, -130); ctx.lineTo(6, -120); ctx.stroke();
  ctx.fillStyle = '#242a31'; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.ellipse(-7, -126, 10, 8.5, -0.2, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* きばの くち */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath();
  ctx.moveTo(4, -112); ctx.quadraticCurveTo(14, -104, 23, -112);
  ctx.stroke();
  ctx.fillStyle = '#fff';
  ctx.beginPath(); ctx.moveTo(9, -111); ctx.lineTo(12, -105); ctx.lineTo(14, -111); ctx.closePath(); ctx.fill();
  ctx.beginPath(); ctx.moveTo(16, -111); ctx.lineTo(19, -105); ctx.lineTo(21, -111); ctx.closePath(); ctx.fill();

  /* ★ピンチの とき、まわりに あおい ひかり */
  if (mad) {
    ctx.save();
    ctx.globalAlpha = 0.35 + 0.25 * Math.abs(Math.sin(t * 6));
    ctx.strokeStyle = '#4fc3f7'; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.ellipse(0, -74, 52, 84, 0, 0, Math.PI * 2); ctx.stroke();
    ctx.restore();
  }
...
/* --- フロギット（1面のざこ）---
     いせきの カエル。おおきな くちと まるい め。ぴょんぴょん はねる。    */
function drawFroggit(ctx, s) {
  const t = (s && s.t) || 0;
  const hop = (s && s.moving) ? Math.abs(Math.sin(t * 5)) * 6 : 0;
  const INK = '#2f2b22', BODY = '#efe7cf', BODY2 = '#d8cdb0';
  ctx.save();
  ctx.lineJoin = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.translate(0, -hop);
  ...
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.ellipse(d * 20, -6 + hop, 12, 6, d * 0.2, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }
  /* どう */
  ctx.fillStyle = BODY;
  ctx.beginPath(); ctx.ellipse(0, -30, 30, 26, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* おおきな くち */
  ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-16, -22); ctx.quadraticCurveTo(4, -10, 24, -24);
  ctx.stroke();
  /* め 2つ（うえに でっぱる） */
  for (const [ex, ey] of [[-8, -54], [14, -56]]) {
    ctx.fillStyle = BODY; ctx.lineWidth = 2.6; ctx.strokeStyle = INK;
    ctx.beginPath(); ctx.arc(ex, ey, 11, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = INK;
    ctx.beginPath(); ctx.arc(ex + 2, ey, 4.4, 0, Math.PI * 2); ctx.fill();
  }
  /* ほっぺの もよう */
  ctx.fillStyle = 'rgba(120,105,70,.35)';
  ctx.beginPath(); ctx.ellipse(-18, -34, 7, 5, 0, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}

/* --- ナプスタブルーク（1面のざこ）---
     しろい ゆうれい。かなしそうな かお。ヘッドホンを して いる。
     ★ゆうれい ぞくせい（ダメージを むこうか する ことが ある）。       */
function drawNapstablook(ctx, s) {
  const t = (s && s.t) || 0;
  ...
  ctx.translate(0, float);
  /* からだ（したが なみなみ） */
  ctx.fillStyle = '#f2f4fb'; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(-26, -34);
  ctx.quadraticCurveTo(-26, -72, 0, -72);
  ctx.quadraticCurveTo(26, -72, 26, -34);
  ctx.lineTo(26, -10);
  ctx.quadraticCurveTo(17, -2, 9, -10);
  ctx.quadraticCurveTo(0, -2, -9, -10);
  ctx.quadraticCurveTo(-17, -2, -26, -10);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* め（したむき の なみだ がた） */
  ctx.fillStyle = INK;
  for (const ex of [-9, 11]) {
    ctx.beginPath();
    ctx.moveTo(ex, -54);
    ctx.quadraticCurveTo(ex + 5, -46, ex, -38);
    ctx.quadraticCurveTo(ex - 5, -46, ex, -54);
    ctx.closePath(); ctx.fill();
  }
  /* ちいさな くち */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.arc(1, -26, 5, Math.PI * 1.15, Math.PI * 1.85); ctx.stroke();
  /* ヘッドホン */
  ctx.strokeStyle = '#5c6478'; ctx.lineWidth = 3.4;
  ctx.beginPath(); ctx.arc(0, -62, 24, Math.PI * 1.15, Math.PI * 1.85); ctx.stroke();
  ctx.fillStyle = '#5c6478'; ctx.strokeStyle = INK; ctx.lineWidth = 2;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.ellipse(d * 23, -58, 5, 8, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }
  ctx.restore();
}

/* --- レッサードッグ（2面のざこ）---
     よろいを きた しろい いぬ。★くびが どんどん のびる★ のが とくちょう。
     ナイフを もって いる。                                            */
function drawLesserDog(ctx, s) {
  const t = (s && s.t) || 0;
  /* くびが のびたり ちぢんだり する */
  const neck = 26 + Math.abs(Math.sin(t * 1.3)) * 44;
  const INK = '#2b2b2b', FUR = '#f6f6f2', ARM = '#9fb0bd';
  ctx.save();
  ctx.lineJoin = 'round'; ctx.lineCap = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  /* あし */
  for (const [x, w] of [[-16, 8], [-4, 8], [10, 8], [20, 8]]) {
    ctx.fillStyle = ARM;
    roundRect(ctx, x - w / 2, -18, w, 18, 3); ctx.fill(); ctx.stroke();
  }
  /* どう（よろい） */
  ctx.fillStyle = ARM;
  roundRect(ctx, -26, -48, 54, 32, 8); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = 'rgba(0,0,0,.25)'; ctx.lineWidth = 2;
  for (const x of [-12, 0, 12]) {
    ctx.beginPath(); ctx.moveTo(x, -46); ctx.lineTo(x, -18); ctx.stroke();
  }
  /* しっぽ */
  ctx.strokeStyle = FUR; ctx.lineWidth = 7;
  ctx.beginPath();
  ctx.moveTo(-26, -40); ctx.quadraticCurveTo(-44, -48, -42, -62);
  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.moveTo(-26, -40); ctx.quadraticCurveTo(-44, -48, -42, -62);
  ctx.stroke();
  /* ★ながい くび */
  ctx.fillStyle = FUR; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  roundRect(ctx, -3, -48 - neck, 18, neck + 10, 8); ctx.fill(); ctx.stroke();
  /* あたま */
  const hy = -50 - neck;
  ctx.beginPath(); ctx.ellipse(8, hy, 18, 15, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* はなさき */
  ctx.beginPath(); ctx.ellipse(24, hy + 3, 9, 7, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.arc(31, hy + 2, 3, 0, Math.PI * 2); ctx.fill();
  /* たれみみ */
  ctx.fillStyle = '#e3e3dc';
  for (const d of [0, 1]) {
    ctx.beginPath();
    ctx.ellipse(-2 + d * 14, hy - 12, 6, 12, d ? 0.3 : -0.3, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }
  /* め */
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(12, hy - 3, 3, 3.6, 0, 0, Math.PI * 2); ctx.fill();
  /* ナイフ */
  ctx.save();
  ctx.translate(30, -30); ctx.rotate(-0.5);
  ctx.fillStyle = '#cfd8dc'; ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.moveTo(-3, 8); ctx.lineTo(3, 8); ctx.lineTo(3, -20); ctx.lineTo(0, -26); ctx.lineTo(-3, -20);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#6d4c41';
  roundRect(ctx, -5, 8, 10, 10, 2); ctx.fill(); ctx.stroke();
  ctx.restore();
  ctx.restore();
}

/* --- スノードレイク（2面のざこ）---
     あたまに ゆきを のせた あおい とり。だじゃれを いう。             */
function drawSnowdrake(ctx, s) {
  const t = (s && s.t) || 0;
  const flap = Math.sin(t * 6) * 0.4;
  const INK = '#22303c', BODY = '#7cc3e8', BODY2 = '#5aa9d2', SNOW = '#f4fbff';
  ctx.save();
  ctx.lineJoin = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  /* あし */
  ctx.strokeStyle = '#e8a33c'; ctx.lineWidth = 4;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.moveTo(d * 7, -22); ctx.lineTo(d * 9, -2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(d * 9, -2); ctx.lineTo(d * 9 + 8, -1); ctx.stroke();
  }
  /* つばさ */
  ctx.save();
  ctx.translate(-16, -48); ctx.rotate(-0.4 + flap);
  ctx.fillStyle = BODY2; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.ellipse(-10, 0, 16, 8, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.restore();
  /* どう */
  ctx.fillStyle = BODY; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath(); ctx.ellipse(0, -44, 22, 25, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* おなかの しろ */
  ctx.fillStyle = '#d7eefb';
  ctx.beginPath(); ctx.ellipse(4, -38, 13, 16, 0, 0, Math.PI * 2); ctx.fill();
  /* あたま */
  ctx.fillStyle = BODY;
  ctx.beginPath(); ctx.ellipse(2, -74, 17, 16, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* くちばし */
  ctx.fillStyle = '#e8a33c';
  ctx.beginPath();
  ctx.moveTo(16, -76); ctx.lineTo(32, -70); ctx.lineTo(16, -64); ctx.closePath();
  ctx.fill(); ctx.stroke();
  /* ★あたまの ゆき */
  ctx.fillStyle = SNOW;
  ctx.beginPath();
  ctx.moveTo(-16, -82);
  ctx.quadraticCurveTo(-10, -98, 2, -94);
  ctx.quadraticCurveTo(12, -102, 19, -84);
  ctx.quadraticCurveTo(6, -78, -16, -82);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* め */
  ctx.fillStyle = INK;
  ctx.beginPath(); ctx.ellipse(10, -76, 3, 4, 0, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}

/* --- アーロン（3面のざこ）---
     サングラスの タツノオトシゴ。ちからこぶを つくって ウィンクして くる。 */
function drawAaron(ctx, s) {
  const t = (s && s.t) || 0;
  const flex = 0.5 + 0.5 * Math.abs(Math.sin(t * 4));
  const INK = '#1e2a33', SKIN = '#7fd0c8', SKIN2 = '#5fb3ab';
  ctx.save();
  ctx.lineJoin = 'round'; ctx.lineCap = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  /* まきついた しっぽ */
  ...
  ctx.beginPath();
  ctx.moveTo(-4, -26);
  ctx.quadraticCurveTo(-26, -20, -22, -6);
  ctx.quadraticCurveTo(-18, 2, -6, -2);
  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.moveTo(-4, -26);
  ...
  for (const d of [-1, 1]) {
    ctx.fillStyle = SKIN;
    ctx.beginPath();
    ctx.ellipse(d * 26, -66 - flex * 6, 12 + flex * 4, 10 + flex * 3, d * 0.4, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
    ctx.strokeStyle = SKIN2; ctx.lineWidth = 6;
    ctx.beginPath();
    ctx.moveTo(d * 18, -58); ctx.lineTo(d * 30, -56 - flex * 4);
    ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  }
  /* せなかの ひれ */
  ctx.fillStyle = SKIN2;
  ctx.beginPath();
  ctx.moveTo(-22, -70); ctx.lineTo(-34, -56); ctx.lineTo(-20, -44); ctx.closePath();
  ctx.fill(); ctx.stroke();
  /* あたま（ながい はな） */
  ctx.fillStyle = SKIN;
  ctx.beginPath(); ctx.ellipse(2, -90, 18, 16, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(14, -96); ctx.quadraticCurveTo(38, -92, 36, -80);
  ctx.quadraticCurveTo(26, -78, 14, -84);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* サングラス */
  ctx.fillStyle = '#20262b';
  roundRect(ctx, -10, -98, 28, 11, 3); ctx.fill();
  ctx.strokeStyle = '#20262b'; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(-10, -94); ctx.lineTo(-18, -92); ctx.stroke();
  /* にやり */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.moveTo(4, -84); ctx.quadraticCurveTo(11, -78, 18, -84);
  ctx.stroke();
  ctx.restore();
}

/* --- マッドダミー（3面のざこ）---
     おこった わたの にんぎょう。ぼうに ささって いて、ゆらゆら する。
     ちいさな にんぎょうを なげて くる。                               */
function drawMadDummy(ctx, s) {
  const t = (s && s.t) || 0;
  const sway = Math.sin(t * 4) * 0.13;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const thr = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#2f2a24', CLOTH = '#d9cdb4', CLOTH2 = '#bfb096';
  ctx.save();
  ctx.lineJoin = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  /* だい */
  ctx.fillStyle = '#6d5b42';
  ctx.beginPath(); ctx.ellipse(0, -4, 22, 7, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#7d6a4d';
  roundRect(ctx, -5, -34, 10, 32, 3); ctx.fill(); ctx.stroke();
  /* ゆれる ほんたい */
  ctx.save();
  ctx.translate(0, -34); ctx.rotate(sway);
  /* うで */
  ctx.fillStyle = CLOTH2;
  for (const d of [-1, 1]) {
    ctx.save(); ctx.translate(d * 24, -34); ctx.rotate(d * (0.5 + thr * 0.6));
    roundRect(ctx, -7, -6, 22, 13, 6); ctx.fill(); ctx.stroke();
    ctx.restore();
  }
  /* どう */
  ctx.fillStyle = CLOTH;
  ctx.beginPath();
  ctx.moveTo(-20, -8);
  ctx.quadraticCurveTo(-24, -44, 0, -46);
  ctx.quadraticCurveTo(24, -44, 20, -8);
  ctx.quadraticCurveTo(0, -2, -20, -8);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* あたま */
  ctx.beginPath(); ctx.ellipse(0, -62, 20, 19, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* ぬいめ */
  ctx.strokeStyle = CLOTH2; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-14, -70); ctx.lineTo(14, -70); ctx.stroke();
  /* おこった かお */
  ctx.fillStyle = INK;
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.moveTo(d * 4, -66); ctx.lineTo(d * 14, -62); ctx.lineTo(d * 5, -58);
    ctx.closePath(); ctx.fill();
  }
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(-9, -48); ctx.quadraticCurveTo(0, -56, 9, -48);
  ctx.stroke();
  ctx.restore();
  /* なげる ちいさな にんぎょう */
  if (thr > 0.2) {
    ctx.save();
    ctx.globalAlpha = thr;
    ctx.translate(38 + thr * 14, -66);
    ctx.scale(0.42, 0.42);
    ctx.fillStyle = CLOTH; ctx.strokeStyle = INK; ctx.lineWidth = 5;
    ctx.beginPath(); ctx.ellipse(0, 0, 22, 24, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = INK;
...
/* ==========================================================================
   ...
   デザインの こだわり
     ・ちいさくて ずんぐり。あたまは おおきい ガイコツ。
     ・いつも にっこり わらって いる（くちは ぜったいに とじない）。
     ・あおい パーカー。フードの ふちは しろい もこもこ。
     ・くろい ハーフパンツに しろい ライン。
     ・ピンクの もこもこ スリッパ（ぜったい くつは はかない）。
     ・りょうては いつも ポケットの なか。やる気が ない。
     ・こうげきの ときだけ かたてを だして、
       ★ガスターブラスター★（ドラゴンの がいこつ）を よびだす。
     ・しんかすると ひだりめが あおく ひかる。
   ========================================================================== */

/* ガスターブラスター（ドラゴンの ような がいこつ の あたま）
   cx,cy … くちの つけね。fire … 0〜1 で ビームの つよさ           */
function drawBlaster(ctx, cx, cy, sc, fire, tint) {
  ctx.save();
  ctx.translate(cx, cy); ctx.scale(sc, sc);
  const INK = '#1b1b22', BONE = '#f7f5ee';
  ctx.lineJoin = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;

  const open = 0.06 + fire * 0.52;     // したあごが ひらく かくど

  /* ビーム（くちから まっすぐ みぎへ。さきほど ひろがる）*/
  if (fire > 0.02) {
    const g = ctx.createLinearGradient(22, 0, 210, 0);
    g.addColorStop(0, tint || 'rgba(255,255,255,0.95)');
    g.addColorStop(0.55, 'rgba(255,255,255,0.45)');
    g.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = g;
    const hh = 4 + fire * 10;
    ctx.beginPath();
    ctx.moveTo(24, -hh); ctx.lineTo(200, -hh * 1.9);
    ctx.lineTo(200, hh * 1.9); ctx.lineTo(24, hh + 6);
    ctx.closePath(); ctx.fill();
  }

  /* つの 2ほん（うしろに ながく のびる）*/
  ctx.fillStyle = BONE;
  for (const [ax, ay, len] of [[-16, -22, 1.0], [-14, -10, 0.76]]) {
    ctx.beginPath();
    ctx.moveTo(ax, ay);
    ctx.quadraticCurveTo(ax - 18 * len, ay - 16 * len, ax - 25 * len, ay - 3 * len);
    ctx.quadraticCurveTo(ax - 14 * len, ay - 2 * len, ax + 2, ay + 8);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  ...
  ctx.beginPath();
  ctx.moveTo(-24, -26);
  ctx.quadraticCurveTo(4, -34, 24, -20);
  ctx.lineTo(26, -6);
  ctx.lineTo(-20, -2);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* うわあごの きば */
  for (const x of [-8, 2, 12]) {
    ctx.beginPath();
    ctx.moveTo(x - 4, -4); ctx.lineTo(x, 7); ctx.lineTo(x + 4, -4);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  /* したあご（うしろを じくに して ひらく）*/
  ctx.save();
  ctx.translate(-20, -2); ctx.rotate(open); ctx.translate(20, 2);
  ctx.fillStyle = BONE;
  ctx.beginPath();
  ctx.moveTo(-18, 2);
  ctx.quadraticCurveTo(2, 15, 24, 5);
  ctx.lineTo(21, 17);
  ctx.lineTo(-16, 15);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* したの きば（うえむき）*/
  for (const x of [-6, 4, 14]) {
    ctx.beginPath();
    ctx.moveTo(x - 4, 6); ctx.lineTo(x, -4); ctx.lineTo(x + 4, 6);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  ctx.restore();

  /* めの あな（からっぽ。うつ ときは ひかる）*/
  ctx.fillStyle = fire > 0.02 ? (tint || '#ffffff') : '#20202a';
  ctx.beginPath(); ctx.ellipse(-4, -22, 8, 6.5, -0.18, 0, Math.PI * 2); ctx.fill();
  if (fire > 0.02) {
    ctx.save(); ctx.globalAlpha = 0.5;
    ctx.fillStyle = tint || '#fff';
    ctx.beginPath(); ctx.ellipse(-4, -22, 15, 12, -0.18, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  ctx.restore();
}

/* サンズ ほんたい。true = しんかご（ひだりめが ひかる）*/
function drawSansBody(ctx, s, evo) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const breathe = Math.sin(t * 1.6) * 1.3;       // ゆっくり ゆれる（やる気ゼロ）
  const INK = '#1b1b22', BONE = '#f7f5ee', HOOD = '#4a7fd6', HOOD2 = '#3a66b0';
  const FUR = '#eceff5', SHORT = '#2a2c36', SLIP = '#f0a7c0';
  const GLOW = '#39c6f5';

  /* ------------------------------------------------------------------
     ★からだの わりあい（だいじ）★
       サンズは ★2〜2.5とうしん★ の ずんぐりむっくり。せは ひくい。
         あし〜かたの したまで ……  0 〜 −62
         あたま（あご −60／てっぺん −114）…… たかさ 54
         ぜんたい 114 ÷ あたま 54 ＝ やく ★2.1とうしん★
       よこはばは あたまと どうが ほぼ おなじ。これで ずんぐり します。
     ------------------------------------------------------------------ */
  ...
  /* ★ガスターブラスター（こうげきの ときだけ。しんかごは 2きに）*/
  if (cast > 0.05) {
    drawBlaster(ctx, 80, -112 - cast * 8, 1.02 + cast * 0.14, cast, evo ? GLOW : '#fff9c4');
    if (evo) drawBlaster(ctx, 68, -38 + cast * 5, 0.80 + cast * 0.10, cast * 0.9, GLOW);
  }

  /* あし（みじかい ほねが ちょっと みえる だけ） */
  ctx.strokeStyle = BONE; ctx.lineWidth = 8;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.moveTo(d * 11, -25); ctx.lineTo(d * 12, -13); ctx.stroke();
  }
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.moveTo(d * 11, -25); ctx.lineTo(d * 12, -13); ctx.stroke();
  }
  /* ★ピンクの もこもこ スリッパ（おおきめ。あしもとを どっしり）*/
  ctx.fillStyle = SLIP; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.ellipse(d * 12 + 4, -7, 16, 7.5, 0, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#ffd3e2';
    ctx.beginPath(); ctx.ellipse(d * 12 - 2, -10, 7.5, 4.5, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = SLIP;
  }

  /* くろい ハーフパンツ（よこに ひろい・しろい ライン） */
  ctx.fillStyle = SHORT; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  roundRect(ctx, -25, -44, 50, 22, 6); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#f2f4f8';
  ctx.fillRect(-25, -40, 50, 3.5);

  /* パーカー（どう。たるを よこに した ような ずんぐりがた） */
  ctx.fillStyle = HOOD; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-32, -70);
  ctx.quadraticCurveTo(-38, -55, -31, -40);
  ctx.lineTo(31, -40);
  ctx.quadraticCurveTo(38, -55, 32, -70);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* まえの あきと ひも */
  ctx.fillStyle = HOOD2;
  ctx.beginPath();
  ctx.moveTo(7, -70); ctx.lineTo(18, -70); ctx.lineTo(16, -42); ctx.lineTo(5, -42);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = FUR; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(3, -65); ctx.lineTo(1, -53); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(12, -65); ctx.lineTo(15, -53); ctx.stroke();

  /* ★ポケットに つっこんだ て（こうげきの ときだけ かたてを だす）*/
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.fillStyle = HOOD2;
  roundRect(ctx, -25, -59, 18, 11, 4); ctx.fill(); ctx.stroke();   // ひだりの ポケット
  if (cast > 0.05) {
    /* みぎては そとに。てのひらを まえに むける（うでも みじかい）*/
    ctx.strokeStyle = HOOD; ctx.lineWidth = 12;
    ctx.beginPath();
    ctx.moveTo(22, -62);
    ctx.lineTo(37 + cast * 7, -58 - cast * 11);
    ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.moveTo(22, -62); ctx.lineTo(37 + cast * 7, -58 - cast * 11); ctx.stroke();
    ctx.fillStyle = BONE;
    ctx.beginPath(); ctx.arc(40 + cast * 7, -57 - cast * 11, 7.5, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  } else {
    ctx.fillStyle = HOOD2;
    roundRect(ctx, 6, -59, 18, 11, 4); ctx.fill(); ctx.stroke();   // みぎの ポケット
  }

  /* フード（くびの うしろの しろい もこもこ。くびは ない）*/
  ctx.fillStyle = FUR; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath();
  ctx.moveTo(-29, -73);
  ctx.quadraticCurveTo(-39, -57, -25, -50);
  ctx.quadraticCurveTo(-12, -58, -10, -71);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(-28, -74);
  ctx.quadraticCurveTo(0, -84, 29, -74);
  ctx.quadraticCurveTo(0, -65, -28, -74);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* ★あたま（おおきい ガイコツ。どうと おなじ くらい よこに ひろい）*/
  ctx.fillStyle = BONE; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-27, -83);
  ctx.quadraticCurveTo(-29, -114, 2, -116);
  ctx.quadraticCurveTo(32, -114, 30, -83);
  ctx.quadraticCurveTo(26, -61, 2, -61);
  ctx.quadraticCurveTo(-23, -61, -27, -83);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* めの あな */
  ctx.fillStyle = '#25252e';
  ctx.beginPath(); ctx.ellipse(1, -95, 8.5, 10, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(20, -94, 8, 9.5, 0, 0, Math.PI * 2); ctx.fill();
  /* しろい ひとみ（しんかまえ／しんかごは みぎだけ） */
  ctx.fillStyle = '#ffffff';
  if (!evo) {
    ctx.beginPath(); ctx.arc(3, -94, 3.5, 0, Math.PI * 2); ctx.fill();
  }
  ctx.beginPath(); ctx.arc(22, -93, 3.3, 0, Math.PI * 2); ctx.fill();

  /* ★しんかご：ひだりめが あおく ひかる */
  if (evo) {
    const fl = 0.75 + 0.25 * Math.abs(Math.sin(t * 7));
    ctx.save();
    ctx.globalAlpha = fl;
    ctx.fillStyle = GLOW;
    ctx.beginPath(); ctx.ellipse(1, -95, 7.5, 9, 0, 0, Math.PI * 2); ctx.fill();
    /* たちのぼる ほのお */
    ctx.beginPath();
    ctx.moveTo(-5, -101);
    ctx.quadraticCurveTo(-12, -122, -2, -115);
    ctx.quadraticCurveTo(2, -130, 6, -113);
    ctx.quadraticCurveTo(12, -122, 6, -101);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#eaf8ff';
    ctx.beginPath(); ctx.arc(1, -96, 3, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }

  /* ★いつも わらって いる くち（ぜったいに とじない） */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath();
  ctx.moveTo(-15, -81);
  ctx.quadraticCurveTo(5, -63, 28, -80);
  ctx.stroke();
  for (const x of [-4, 4, 12, 20]) {
    ctx.beginPath(); ctx.moveTo(x, -79); ctx.lineTo(x, -72.5); ctx.stroke();
  }
  /* ほほの かげ（まるい かおを だす） */
  ctx.fillStyle = 'rgba(180,170,150,.20)';
  ctx.beginPath(); ctx.ellipse(-13, -78, 6.5, 4.5, 0, 0, Math.PI * 2); ctx.fill();

  ctx.restore();
}

function drawSans(ctx, s)     { drawSansBody(ctx, s, false); }
function drawSansTrue(ctx, s) { drawSansBody(ctx, s, true); }

/* ==========================================================================
   ★ とくせつ「地底の国」4〜7ステージの キャラたち
   ========================================================================== */

/* --- ヴァルキン（4ステージの ざこ）---
     ちいさな かざん。あたまの クレーターから ようがんが あふれる。
     わるぎは まったく ないのに、ちかづくと あつい。                   */
function drawVulkin(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#2b2028', ROCK = '#7d6a62', ROCK2 = '#5f4f49';
  const LAVA = '#ff7043', LAVA2 = '#ffca28';
  ...
  ctx.beginPath();
  ctx.moveTo(-17, -52);
  ctx.quadraticCurveTo(-24, -26, -28, 0);
  ctx.lineTo(28, 0);
  ctx.quadraticCurveTo(24, -26, 17, -52);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* すその かげ */
  ctx.fillStyle = ROCK2;
  ctx.beginPath();
  ctx.moveTo(-26, -12); ctx.lineTo(26, -12); ctx.lineTo(28, 0); ctx.lineTo(-28, 0);
  ctx.closePath(); ctx.fill();
  ...
  for (const d of [-1, 1]) {
    ctx.beginPath();
    ctx.ellipse(d * 27, -22 - cast * 6 * d, 8, 6, d * 0.4, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }

  /* クレーター（ふちと ようがん） */
  ctx.fillStyle = ROCK2;
  ctx.beginPath(); ctx.ellipse(0, -52, 18, 6.5, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = LAVA;
  ctx.beginPath(); ctx.ellipse(0, -52, 13, 4.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = LAVA2;
  ctx.beginPath(); ctx.ellipse(-2, -53, 6, 2.2, 0, 0, Math.PI * 2); ctx.fill();
  ...
  if (cast > 0.1) {
    ...
    ctx.beginPath();
    ctx.moveTo(-9, -54);
    ctx.quadraticCurveTo(-4, -70 - cast * 26, 1, -60);
    ctx.quadraticCurveTo(5, -76 - cast * 30, 9, -54);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = LAVA2;
    for (let i = 0; i < 3; i++) {
      const a = t * 4 + i * 2.1;
      ctx.beginPath();
      ctx.arc(Math.sin(a) * 14, -62 - cast * (18 + i * 9), 3 - i * 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  /* ようがんの たれ（よこから つたう） */
  ctx.strokeStyle = LAVA; ctx.lineWidth = 3.4;
  ctx.beginPath(); ctx.moveTo(-16, -50); ctx.lineTo(-22, -34 - Math.sin(t * 2) * 2); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(17, -50);  ctx.lineTo(24, -30 - Math.cos(t * 2) * 2);  ctx.stroke();

  /* かお（いつも にこにこ） */
  ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  for (const x of [-9, 9]) {
    ctx.beginPath();
    ctx.moveTo(x - 4.5, -30); ctx.lineTo(x, -34.5); ctx.lineTo(x + 4.5, -30);
    ctx.stroke();
  }
  ctx.fillStyle = INK;
  ctx.beginPath();
  ctx.moveTo(-8, -25); ctx.quadraticCurveTo(0, -14, 8, -25);
  ctx.quadraticCurveTo(0, -20, -8, -25);
  ctx.closePath(); ctx.fill();
  /* ほっぺ */
  ctx.fillStyle = 'rgba(255,112,67,.35)';
...
/* --- マッフェット（4ステージの ざこ）---
     ...
     ★クモの巣を はって、みかたを ずっと どんそくに します。          */
function drawMuffet(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#2a1f33', SKIN = '#b39ddb', SKIN2 = '#9a80c6';
  const DRESS = '#6b3fa0', DRESS2 = '#54307f', HAIR = '#3b2352';
  ...
  /* ぶらさがって いる いと */
  ctx.strokeStyle = 'rgba(236,225,245,.55)'; ctx.lineWidth = 1.6;
  ctx.beginPath(); ctx.moveTo(-2, -104); ctx.lineTo(-2, -150); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ...
  ctx.beginPath();
  ctx.moveTo(-15, -56);
  ctx.quadraticCurveTo(-26, -26, -30, 0);
  ctx.lineTo(30, 0);
  ctx.quadraticCurveTo(26, -26, 15, -56);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = DRESS2;
  ctx.beginPath();
  ctx.moveTo(-28, -12); ctx.lineTo(28, -12); ctx.lineTo(30, 0); ctx.lineTo(-30, 0);
  ctx.closePath(); ctx.fill();

  /* うしろの うで 2ほん（かげ） */
  ctx.strokeStyle = SKIN2; ctx.lineWidth = 6;
  ctx.beginPath(); ctx.moveTo(-6, -74); ctx.quadraticCurveTo(-24, -66, -30, -52); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(8, -74);  ctx.quadraticCurveTo(26, -68, 32, -56);  ctx.stroke();

  /* どう */
  ctx.fillStyle = DRESS; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  roundRect(ctx, -15, -84, 30, 30, 8); ctx.fill(); ctx.stroke();
  /* えりの リボン */
  ctx.fillStyle = '#f06292';
  ctx.beginPath();
  ctx.moveTo(0, -82); ctx.lineTo(-9, -88); ctx.lineTo(-9, -76); ctx.closePath(); ctx.fill();
  ctx.beginPath();
  ctx.moveTo(0, -82); ctx.lineTo(9, -88); ctx.lineTo(9, -76); ctx.closePath(); ctx.fill();

  /* まえの うで 4ほん（みぎがわに 2、ひだりがわに 2）*/
  ctx.strokeStyle = SKIN; ctx.lineWidth = 6.5;
  const sw = Math.sin(t * 2.4) * 3;
  const arms = [
    [14, -80,  34 + cast * 10, -76 - cast * 12],
    [14, -70,  32,             -54 + sw],
    [-14, -80, -32,            -72 - sw],
    [-14, -68, -30,            -50],
  ];
  for (const [x1, y1, x2, y2] of arms) {
    ctx.strokeStyle = SKIN; ctx.lineWidth = 6.5;
    ctx.beginPath(); ctx.moveTo(x1, y1); ctx.quadraticCurveTo((x1 + x2) / 2, y1 - 6, x2, y2); ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 1.8;
    ctx.beginPath(); ctx.moveTo(x1, y1); ctx.quadraticCurveTo((x1 + x2) / 2, y1 - 6, x2, y2); ctx.stroke();
    ctx.fillStyle = SKIN; ctx.strokeStyle = INK; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(x2, y2, 4.6, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }

  /* ★こうげき：クモの巣 の たまを なげる */
  if (cast > 0.12) {
    ctx.save();
    ctx.globalAlpha = 0.85 * cast;
    const bx = 46 + cast * 16, by = -78 - cast * 14, r = 7 + cast * 5;
    ctx.fillStyle = '#efe6f7'; ctx.strokeStyle = '#b39ddb'; ctx.lineWidth = 1.6;
    ctx.beginPath(); ctx.arc(bx, by, r, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    for (let i = 0; i < 6; i++) {
      const a = i * Math.PI / 3 + t;
      ctx.beginPath(); ctx.moveTo(bx, by);
      ctx.lineTo(bx + Math.cos(a) * r, by + Math.sin(a) * r); ctx.stroke();
    }
    ctx.restore();
  }

  /* あたま */
  ctx.fillStyle = SKIN; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath(); ctx.ellipse(0, -100, 17, 16, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* ツインテール（りょうがわの おだんご） */
  ctx.fillStyle = HAIR;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.arc(d * 20, -108, 9, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }
  /* まえがみ */
  ctx.fillStyle = HAIR;
  ctx.beginPath();
  ctx.moveTo(-17, -104);
  ctx.quadraticCurveTo(0, -122, 17, -104);
  ctx.quadraticCurveTo(0, -110, -17, -104);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* ★めが 5つ（うえに 3・したに 2）*/
  ctx.fillStyle = '#20182a';
  for (const [x, y, r] of [[-9, -102, 2.6], [0, -103, 2.9], [9, -102, 2.6],
                           [-5, -95, 2.3], [6, -95, 2.3]]) {
    ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill();
  }
  ctx.fillStyle = '#fff';
  ctx.beginPath(); ctx.arc(1, -104, 1.1, 0, Math.PI * 2); ctx.fill();
  /* にやりと わらう くち */
  ctx.strokeStyle = '#6d2b52'; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(-6, -89); ctx.quadraticCurveTo(0, -84, 7, -90); ctx.stroke();

  ctx.restore();
}

/* --- おうこくけいびたい（4〜6ステージの ざこ。01と02）---
     よろいの ふたりぐみ。01は うさぎ、02は ねこ。
     ほんとうは なかよしなので、02の あたまの うえには
     ときどき ハートが でます。                                        */
function drawGuardBody(ctx, s, two) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#22242c', ARM = two ? '#9aa7b4' : '#8792a0', ARM2 = two ? '#77828f' : '#67717d';
  const CLOTH = two ? '#c05a6a' : '#4d6fa8', SKIN = '#f2ece2';
  const H = two ? 0.88 : 1.0;                   // 02は すこし ちいさい
  ...
  ctx.fillStyle = ARM2;
  for (const d of [-1, 1]) {
    roundRect(ctx, d * 12 - 8, -26, 16, 26, 5); ctx.fill(); ctx.stroke();
  }

  /* こしの ぬの */
  ctx.fillStyle = CLOTH;
  ctx.beginPath();
  ctx.moveTo(-20, -46); ctx.lineTo(20, -46); ctx.lineTo(16, -18); ctx.lineTo(-16, -18);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ...
  ctx.beginPath();
  ctx.moveTo(-22, -88);
  ctx.quadraticCurveTo(-26, -66, -20, -44);
  ctx.lineTo(20, -44);
  ctx.quadraticCurveTo(26, -66, 22, -88);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* むねの ライン */
  ctx.strokeStyle = ARM2; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(0, -86); ctx.lineTo(0, -46); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(-18, -68); ctx.lineTo(18, -68); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 3;

  /* かたあて */
  ctx.fillStyle = ARM2;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.ellipse(d * 24, -84, 11, 8.5, d * 0.3, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }

  /* ★ぶき（01は おおきな けん・02は おの）*/
  ctx.save();
  ctx.translate(26, -74);
  ctx.rotate(-0.5 - cast * 1.5);
  ctx.strokeStyle = '#6d5844'; ctx.lineWidth = 6;
  ...
  if (!two) {                       /* けん */
    ctx.beginPath();
    ctx.moveTo(-6, -22); ctx.lineTo(6, -22); ctx.lineTo(4, -62); ctx.lineTo(0, -70);
    ctx.lineTo(-4, -62); ctx.closePath(); ctx.fill(); ctx.stroke();
  } else {                          /* おの */
    ctx.beginPath();
    ctx.moveTo(-2, -24);
    ctx.quadraticCurveTo(-26, -34, -20, -56);
    ctx.quadraticCurveTo(-2, -48, 2, -52);
    ctx.quadraticCurveTo(22, -56, 18, -34);
  ...
  /* うで */
  ctx.strokeStyle = ARM; ctx.lineWidth = 9;
  ctx.beginPath(); ctx.moveTo(18, -82); ctx.lineTo(26, -74); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(18, -82); ctx.lineTo(26, -74); ctx.stroke();
  ...
  ctx.beginPath();
  ctx.moveTo(-18, -96);
  ctx.quadraticCurveTo(-20, -122, 0, -124);
  ctx.quadraticCurveTo(20, -122, 18, -96);
  ctx.quadraticCurveTo(0, -90, -18, -96);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* めの すきま */
  ctx.fillStyle = '#1b1d24';
  roundRect(ctx, -13, -110, 26, 8, 3); ctx.fill();
  ctx.fillStyle = two ? '#ffe082' : '#8fd3ff';
  ctx.beginPath(); ctx.arc(-5, -106, 2.4, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(6, -106, 2.4, 0, Math.PI * 2); ctx.fill();

  /* ★みみ（01は うさぎの ながみみ・02は ねこみみ）*/
  ctx.fillStyle = SKIN; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  if (!two) {
    for (const [d, lean] of [[-1, -0.2], [1, 0.25]]) {
      ctx.save();
      ctx.translate(d * 8, -122); ctx.rotate(lean + Math.sin(t * 1.8) * 0.06);
      ctx.beginPath(); ctx.ellipse(0, -18, 6, 20, 0, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
      ctx.restore();
    }
  } else {
    for (const d of [-1, 1]) {
      ctx.beginPath();
      ctx.moveTo(d * 6, -120); ctx.lineTo(d * 15, -140); ctx.lineTo(d * 19, -118);
      ctx.closePath(); ctx.fill(); ctx.stroke();
    }
    /* なかよしの しるし（ときどき ハート）*/
    const hb = Math.sin(t * 1.1);
    if (hb > 0.6) {
      ctx.save(); ctx.globalAlpha = (hb - 0.6) / 0.4;
      ctx.fillStyle = '#ff6d9e';
      ctx.translate(-26, -140); ctx.scale(0.5, 0.5);
      heartShape(ctx, 0, 0, 16);
      ctx.fill();
      ctx.restore();
    }
  }
...
function drawGuard01(ctx, s) { drawGuardBody(ctx, s, false); }
function drawGuard02(ctx, s) { drawGuardBody(ctx, s, true); }

/* ハートの かたち（たましい・けいびたいの ハートに つかう）*/
function heartShape(ctx, cx, cy, r) {
  ctx.beginPath();
  ctx.moveTo(cx, cy + r * 0.85);
  ctx.bezierCurveTo(cx - r * 1.4, cy - r * 0.25, cx - r * 0.55, cy - r * 1.15, cx, cy - r * 0.42);
  ctx.bezierCurveTo(cx + r * 0.55, cy - r * 1.15, cx + r * 1.4, cy - r * 0.25, cx, cy + r * 0.85);
  ctx.closePath();
}

/* --- にんげんの たましい（7ステージの ざこ）---
     フラウィーに とじこめられた 6つの たましい。
     いろは ゆっくり かわります（ゆうき・しんねん・…）。              */
function drawHumanSoul(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  /* 6つの たましいの いろ */
  const COLORS = ['#ff8a2b', '#2b6bff', '#2bd6ff', '#7bd93b', '#a35bd6', '#ffe14d'];
  const k = Math.floor(t / 2.6) % 6;
  const c = COLORS[(k + 6) % 6];
  const float = Math.sin(t * 2.2) * 3;

  ctx.save();
  ctx.translate(0, -44 + float);
  ctx.lineJoin = 'round';

  /* ぼんやり ひかる */
  const gr = 26 + cast * 10;
  const gg = ctx.createRadialGradient(0, 0, 3, 0, 0, gr);
  gg.addColorStop(0, c);
  gg.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.save();
  ctx.globalAlpha = 0.55 + cast * 0.25;
  ctx.fillStyle = gg;
  ctx.beginPath(); ctx.arc(0, 0, gr, 0, Math.PI * 2); ctx.fill();
  ctx.restore();

  ctx.fillStyle = c; ctx.strokeStyle = '#2a2230'; ctx.lineWidth = 2.6;
  heartShape(ctx, 0, 0, 15);
  ctx.fill(); ctx.stroke();
  /* ハイライト */
  ctx.fillStyle = 'rgba(255,255,255,.65)';
  ctx.beginPath(); ctx.ellipse(-5, -6, 3.6, 2.6, -0.5, 0, Math.PI * 2); ctx.fill();
  ...
  if (cast > 0.15) {
    ...
    for (let i = 0; i < 3; i++) {
      ctx.beginPath();
      ctx.arc(24 + i * 13 + cast * 14, -2 + Math.sin(t * 6 + i) * 4, 3.4 - i * 0.6, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
...
/* --- メタトンEX（4ステージ ボス）---
     ちていの テレビスター。ロボットなのに あしが ながい。
     かたほうの めを かみで かくして、いつも ポーズを きめて いる。   */
function drawMetatonEx(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#1d1a22', BODY = '#2f2b38', BODY2 = '#413c4d';
  const SKIN = '#f6eef4', HAIR = '#241f2c', PINK = '#ff4f9a', GLOW = '#ff2e7e';
  const pose = Math.sin(t * 1.5) * 2;

  ctx.save();
  ctx.lineJoin = 'round'; ctx.lineCap = 'round';
  ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.translate(0, pose);

  /* ★キラキラ（スターの まわり）*/
  ctx.save();
  for (let i = 0; i < 5; i++) {
    const a = t * 1.3 + i * 1.256;
    const rx = Math.cos(a) * 52, ry = -70 + Math.sin(a * 1.3) * 46;
    ctx.globalAlpha = 0.25 + 0.35 * Math.abs(Math.sin(t * 3 + i));
    ctx.fillStyle = i % 2 ? '#ffe082' : PINK;
    ctx.beginPath();
    ctx.moveTo(rx, ry - 6); ctx.lineTo(rx + 2, ry - 2); ctx.lineTo(rx + 6, ry);
    ctx.lineTo(rx + 2, ry + 2); ctx.lineTo(rx, ry + 6); ctx.lineTo(rx - 2, ry + 2);
    ctx.lineTo(rx - 6, ry); ctx.lineTo(rx - 2, ry - 2);
    ctx.closePath(); ctx.fill();
  }
  ctx.restore();
  ...
  ctx.beginPath(); ctx.moveTo(-8, -60); ctx.lineTo(-16, -26); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(9, -60);  ctx.lineTo(16, -26);  ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(-8, -60); ctx.lineTo(-16, -26); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(9, -60);  ctx.lineTo(16, -26);  ctx.stroke();
  ctx.fillStyle = PINK; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  for (const [x] of [[-16], [16]]) {
    ctx.beginPath();
    ctx.moveTo(x - 8, -28); ctx.lineTo(x + 8, -28);
    ctx.lineTo(x + 13, 0);  ctx.lineTo(x - 9, 0);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  ...
  ctx.beginPath();
  ctx.moveTo(-20, -104);
  ctx.quadraticCurveTo(-26, -82, -17, -58);
  ctx.lineTo(18, -58);
  ctx.quadraticCurveTo(27, -82, 21, -104);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = BODY2;
  ctx.beginPath(); ctx.ellipse(0, -66, 16, 7, 0, 0, Math.PI * 2); ctx.fill();
  /* ★ハートの コア（うつ ときに ひかる）*/
  ctx.save();
  ctx.globalAlpha = 0.55 + cast * 0.45;
  ctx.fillStyle = GLOW;
  heartShape(ctx, 1, -84, 13 + cast * 3);
  ctx.fill();
  ctx.strokeStyle = '#ffd2e4'; ctx.lineWidth = 2; ctx.stroke();
  ctx.restore();

  /* かたあて */
  ctx.fillStyle = BODY2; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.ellipse(d * 23, -102, 10, 8, d * 0.25, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }

  /* うで（ひだりは こしに あて、みぎは ポーズ／こうげき）*/
  ctx.strokeStyle = BODY; ctx.lineWidth = 7.5;
  ctx.beginPath(); ctx.moveTo(-22, -100); ctx.quadraticCurveTo(-36, -86, -24, -70); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2; ctx.beginPath();
  ctx.moveTo(-22, -100); ctx.quadraticCurveTo(-36, -86, -24, -70); ctx.stroke();
  const hx = 36 + cast * 16, hy = -118 - cast * 14;
  ctx.strokeStyle = BODY; ctx.lineWidth = 7.5;
  ctx.beginPath(); ctx.moveTo(22, -100); ctx.quadraticCurveTo(34, -114, hx, hy); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(22, -100); ctx.quadraticCurveTo(34, -114, hx, hy); ctx.stroke();
  /* しろい てぶくろ */
  ctx.fillStyle = '#f7f3f8'; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.arc(-25, -68, 6.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ...
  if (cast > 0.12) {
    ...
    ctx.fillStyle = PINK;
    for (let i = 0; i < 4; i++) {
      const bx = 50 + i * 18 + cast * 18, by = -96 + Math.sin(t * 7 + i * 1.4) * 14;
      heartShape(ctx, bx, by, 6.5 - i * 0.8);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  /* あたま */
  ctx.fillStyle = SKIN; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  ctx.beginPath(); ctx.ellipse(1, -126, 16, 18, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  /* かみ（かたほうの めを かくす ながい まえがみ）*/
  ctx.fillStyle = HAIR;
  ctx.beginPath();
  ctx.moveTo(-16, -128);
  ctx.quadraticCurveTo(-18, -150, 2, -146);
  ctx.quadraticCurveTo(20, -144, 17, -124);
  ctx.quadraticCurveTo(6, -134, -4, -120);
  ctx.quadraticCurveTo(-10, -108, -16, -128);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* みえて いる ほうの め（まつげ つき）*/
  ctx.fillStyle = '#20182a';
  ctx.beginPath(); ctx.ellipse(10, -128, 4.2, 5.4, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#fff';
  ctx.beginPath(); ctx.arc(11.4, -130, 1.5, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = INK; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(15, -134); ctx.lineTo(20, -137); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(15, -131); ctx.lineTo(21, -131); ctx.stroke();
  /* くちべに */
  ctx.strokeStyle = PINK; ctx.lineWidth = 2.6;
  ctx.beginPath(); ctx.moveTo(4, -116); ctx.quadraticCurveTo(10, -112, 15, -117); ctx.stroke();
...
/* --- アズゴア（6ステージ ボス）---
     ...
     きんの よろいと むらさきの マント。さんさの やりを もつ。       */
function drawAsgore(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const INK = '#241c2e', CAPE = '#5b32a8', CAPE2 = '#472785';
  const GOLD = '#e3b545', GOLD2 = '#bb8f2c', FUR = '#f6f0e3', FUR2 = '#ddd4c2';
  ...
  ctx.beginPath();
  ctx.moveTo(-26, -112);
  ctx.quadraticCurveTo(-58, -60, -52, 0);
  ctx.lineTo(30, 0);
  ctx.quadraticCurveTo(24, -70, 20, -112);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = CAPE2;
  ctx.beginPath();
  ctx.moveTo(-50, -16); ctx.lineTo(28, -16); ctx.lineTo(30, 0); ctx.lineTo(-52, 0);
  ctx.closePath(); ctx.fill();

  /* どう（きんの よろい）*/
  ctx.fillStyle = GOLD; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-26, -112);
  ctx.quadraticCurveTo(-32, -80, -24, -46);
  ctx.lineTo(24, -46);
  ctx.quadraticCurveTo(32, -80, 26, -112);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* デルタルーンの もんしょう */
  ctx.fillStyle = '#f3ead6';
  ctx.beginPath(); ctx.arc(0, -84, 13, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = GOLD;
  ctx.beginPath(); ctx.arc(0, -84, 8.5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#f3ead6';
  ctx.beginPath();
  ctx.moveTo(0, -104); ctx.lineTo(12, -88); ctx.lineTo(-12, -88); ctx.closePath(); ctx.fill();
  /* こしの おび */
  ctx.fillStyle = GOLD2;
  ctx.beginPath();
  ctx.moveTo(-25, -56); ctx.lineTo(25, -56); ctx.lineTo(24, -46); ctx.lineTo(-24, -46);
  ctx.closePath(); ctx.fill();

  /* かたあて */
  ctx.fillStyle = GOLD2; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
  for (const d of [-1, 1]) {
    ctx.beginPath(); ctx.ellipse(d * 29, -110, 13, 10, d * 0.28, 0, Math.PI * 2);
    ctx.fill(); ctx.stroke();
  }
  /* あたま（やぎ） */
  ctx.fillStyle = FUR; ctx.strokeStyle = INK; ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-22, -130);
  ctx.quadraticCurveTo(-24, -158, 2, -160);
  ctx.quadraticCurveTo(30, -158, 30, -130);
  ctx.quadraticCurveTo(30, -114, 2, -114);
  ctx.quadraticCurveTo(-20, -114, -22, -130);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* ★つの（おおきく うしろに そる）*/
  ctx.fillStyle = FUR2;
  for (const [x, d] of [[-16, -1], [22, 1]]) {
    ctx.beginPath();
    ctx.moveTo(x, -150);
    ctx.quadraticCurveTo(x + d * 16, -182, x + d * 44, -178);
    ctx.quadraticCurveTo(x + d * 26, -166, x + d * 14, -142);
    ctx.closePath(); ctx.fill(); ctx.stroke();
  }
  /* ひげ（あごの したに ふさっと）*/
  ctx.fillStyle = FUR;
  ctx.beginPath();
  ctx.moveTo(-10, -122);
  ctx.quadraticCurveTo(-14, -96, 4, -100);
  ctx.quadraticCurveTo(22, -96, 18, -122);
  ctx.closePath(); ctx.fill(); ctx.stroke();
  /* め（やさしいのに かなしい） */
  ctx.fillStyle = '#2a2230';
  ctx.beginPath(); ctx.ellipse(2, -136, 3.4, 4.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(20, -136, 3.4, 4.2, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(-3, -144); ctx.lineTo(7, -142); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(25, -142); ctx.lineTo(16, -144); ctx.stroke();
  /* はな */
  ctx.fillStyle = '#e8b6b0';
  ctx.beginPath(); ctx.ellipse(27, -128, 4, 3, 0, 0, Math.PI * 2); ctx.fill();

  /* おうかん */
  ctx.fillStyle = '#ffd54f'; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(-10, -158); ctx.lineTo(-12, -176); ctx.lineTo(-3, -168); ctx.lineTo(4, -180);
  ctx.lineTo(11, -168); ctx.lineTo(20, -176); ctx.lineTo(18, -158);
  ctx.closePath(); ctx.fill(); ctx.stroke();

  /* ★うでと さんさの やり（いちばん まえに かく）*/
  ctx.strokeStyle = FUR; ctx.lineWidth = 12;
  ctx.beginPath(); ctx.moveTo(26, -104); ctx.lineTo(52 + cast * 18, -84 - cast * 8); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(26, -104); ctx.lineTo(52 + cast * 18, -84 - cast * 8); ctx.stroke();
  ctx.save();
  ctx.translate(54 + cast * 18, -84 - cast * 8);
  ctx.rotate(-0.16 + cast * 0.30);
  ctx.strokeStyle = '#6d5844'; ctx.lineWidth = 7;
  ctx.beginPath(); ctx.moveTo(0, 74); ctx.lineTo(0, -38); ctx.stroke();
  ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
  ctx.beginPath(); ctx.moveTo(0, 74); ctx.lineTo(0, -38); ctx.stroke();
  ctx.fillStyle = GOLD; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
  ctx.beginPath();
  ctx.moveTo(-15, -34); ctx.lineTo(15, -34); ctx.lineTo(13, -44); ctx.lineTo(18, -70);
  ctx.lineTo(10, -48); ctx.lineTo(3, -76); ctx.lineTo(-4, -48); ctx.lineTo(-12, -70);
  ctx.lineTo(-7, -44); ctx.closePath(); ctx.fill(); ctx.stroke();
  /* て（やりを にぎる）*/
  ctx.fillStyle = FUR; ctx.strokeStyle = INK; ctx.lineWidth = 2.4;
  ...
  if (cast > 0.15) {
    ...
    ctx.fillStyle = '#ff7043';
    ctx.beginPath();
    ctx.moveTo(1, -76); ctx.quadraticCurveTo(-12, -98, -2, -90);
    ctx.quadraticCurveTo(2, -114, 8, -88);
    ctx.quadraticCurveTo(16, -100, 6, -76);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#ffe082';
    ctx.beginPath(); ctx.arc(3, -86, 4, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  ctx.restore();
...
/* --- フラウィー（7ステージ ボス。2だんかい）---
     まえはん：きんいろの はな。にこにこ して いるが めが わらって いない。
     ★たいりょく はんぶんで「オメガフラウィー」に なる（s.enraged）。
       きかいと つるの かたまりに なって、おおきな くちで かみつく。   */
function drawFlowey(ctx, s) {
  const t = (s && s.t) || 0;
  const atk = (s && s.atk !== undefined) ? s.atk : -1;
  const cast = (atk >= 0) ? (1 - Math.abs(atk - 0.5) * 2) : 0;
  const omega = !!(s && s.enraged);
  const INK = '#221f18', STEM = '#4c9a3c', STEM2 = '#3a7a2e';
  const PETAL = '#ffd23f', PETAL2 = '#f0a81e', FACE = '#fff6d8';
  ...
  if (!omega) {
    ...
    /* くき */
    ctx.strokeStyle = STEM; ctx.lineWidth = 9;
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(sway * 0.5, -40, sway, -74);
    ctx.stroke();
    ctx.strokeStyle = INK; ctx.lineWidth = 2.2;
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ...
    ctx.stroke();
    /* は */
    ctx.fillStyle = STEM2; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
    for (const [d, y] of [[-1, -42], [1, -56]]) {
      ctx.beginPath();
      ctx.ellipse(d * 18, y, 17, 8, d * 0.4, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
    }

    /* はなびら */
    ctx.fillStyle = PETAL; ctx.strokeStyle = INK; ctx.lineWidth = 2.8;
    for (let i = 0; i < 8; i++) {
      const a = i * Math.PI / 4 + t * 0.2;
      ctx.save();
      ctx.translate(sway, -96); ctx.rotate(a);
      ctx.beginPath(); ctx.ellipse(0, -25, 11, 15, 0, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
      ctx.restore();
    }
    ctx.fillStyle = PETAL2;
    ctx.beginPath(); ctx.arc(sway, -96, 24, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = FACE;
    ctx.beginPath(); ctx.arc(sway, -96, 20, 0, Math.PI * 2); ctx.fill();

    /* かお（うつ ときは いじわるな かお に なる）*/
    ctx.fillStyle = '#20201a';
    if (cast > 0.3) {
      /* ★いじわるがお：めが つりあがり、ギザギザの くち */
      for (const [x, d] of [[-8, 1], [8, -1]]) {
        ctx.beginPath();
        ctx.moveTo(sway + x - 5 * d, -104); ctx.lineTo(sway + x + 5 * d, -100);
        ctx.lineTo(sway + x + 3 * d, -94); ctx.lineTo(sway + x - 4 * d, -98);
        ctx.closePath(); ctx.fill();
      }
      ctx.beginPath();
      ctx.moveTo(sway - 12, -90);
      for (let i = 0; i < 6; i++) {
        ctx.lineTo(sway - 12 + i * 4 + 2, -84 + (i % 2 ? 0 : 5));
      }
      ctx.lineTo(sway + 13, -90);
      ctx.closePath(); ctx.fill();
    } else {
      ctx.beginPath(); ctx.arc(sway - 7, -101, 3.6, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(sway + 7, -101, 3.6, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = '#20201a'; ctx.lineWidth = 2.6;
      ctx.beginPath();
      ctx.moveTo(sway - 9, -91); ctx.quadraticCurveTo(sway, -84, sway + 9, -91);
      ctx.stroke();
      ctx.fillStyle = 'rgba(240,168,30,.45)';
      ctx.beginPath(); ctx.arc(sway - 14, -94, 4, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(sway + 14, -94, 4, 0, Math.PI * 2); ctx.fill();
    }
    ...
    if (cast > 0.12) {
      ctx.save(); ctx.globalAlpha = cast;
      ctx.fillStyle = '#fff6d8'; ctx.strokeStyle = '#c9a227'; ctx.lineWidth = 2;
      for (let i = 0; i < 4; i++) {
        ctx.beginPath();
        ctx.arc(38 + i * 16 + cast * 14, -96 + Math.sin(t * 6 + i * 1.6) * 16, 6 - i * 0.7, 0, Math.PI * 2);
        ctx.fill(); ctx.stroke();
      }
      ctx.restore();
    }
    ...
    /* ================= こうはん：オメガフラウィー ================= */
    const jit = Math.sin(t * 13) * 1.6;
    ctx.translate(jit, 0);

    /* つるの かたまり（したの ほう）*/
    ctx.strokeStyle = STEM2; ctx.lineWidth = 10;
    for (let i = 0; i < 5; i++) {
      const d = (i - 2) * 16;
      ctx.beginPath();
      ctx.moveTo(d, 0);
      ctx.quadraticCurveTo(d + Math.sin(t * 2 + i) * 16, -44, d * 0.5, -78);
      ctx.stroke();
    }

    /* きかいの からだ（テレビの かたまり）*/
    ctx.fillStyle = '#3c3a44'; ctx.strokeStyle = INK; ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(-52, -74);
    ctx.quadraticCurveTo(-62, -128, -30, -150);
    ctx.lineTo(34, -150);
    ctx.quadraticCurveTo(64, -126, 54, -74);
    ctx.quadraticCurveTo(0, -60, -52, -74);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    /* かたの ケーブル */
    ctx.strokeStyle = '#26242c'; ctx.lineWidth = 5;
    for (const [x1, y1, x2, y2] of [[-48, -120, -70, -96], [50, -120, 72, -100]]) {
      ctx.beginPath(); ctx.moveTo(x1, y1);
      ctx.quadraticCurveTo((x1 + x2) / 2, y1 + 16, x2, y2); ctx.stroke();
    }

    /* テレビの がめん（3まい。ノイズが はしる）*/
    for (const [sx, sy, sw2, sh] of [[-44, -150, 26, 20], [30, -152, 26, 20], [-16, -74, 32, 16]]) {
      ctx.fillStyle = '#10121a'; ctx.strokeStyle = '#7a7a88'; ctx.lineWidth = 2.4;
      roundRect(ctx, sx, sy, sw2, sh, 3); ctx.fill(); ctx.stroke();
      ctx.save(); ctx.globalAlpha = 0.55;
      ctx.fillStyle = '#6de3ff';
      for (let i = 0; i < 4; i++) {
        const yy = sy + 3 + ((i * 5 + t * 34) % (sh - 5));
        ctx.fillRect(sx + 2, yy, sw2 - 4, 1.6);
      }
      ctx.restore();
    }

    /* ★おおきな くち（ギザギザの は）*/
    ctx.fillStyle = '#1a1520'; ctx.strokeStyle = INK; ctx.lineWidth = 3;
    const open = 8 + cast * 20;
    ctx.beginPath();
    ctx.moveTo(-30, -108);
    ctx.quadraticCurveTo(2, -100 + open, 36, -108);
    ctx.quadraticCurveTo(2, -118 - open * 0.5, -30, -108);
    ctx.closePath(); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#f2ead6';
    for (let i = 0; i < 7; i++) {
      const x = -26 + i * 9.5;
      ctx.beginPath();
      ctx.moveTo(x, -114); ctx.lineTo(x + 4.5, -106 + open * 0.3); ctx.lineTo(x + 9, -114);
      ctx.closePath(); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(x, -102 + open * 0.6); ctx.lineTo(x + 4.5, -110); ctx.lineTo(x + 9, -102 + open * 0.6);
      ctx.closePath(); ctx.fill();
    }

    /* ★おおきな め（ひとつだけ。ぎょろり）*/
    const ex = 6 + Math.sin(t * 2.4) * 3;
    ctx.fillStyle = '#f7f3e6'; ctx.strokeStyle = INK; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.ellipse(4, -136, 19, 15, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#20201a';
    ctx.beginPath(); ctx.arc(4 + ex, -136, 7.5, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#ff5252';
    ctx.beginPath(); ctx.arc(4 + ex, -136, 3.2, 0, Math.PI * 2); ctx.fill();

    /* のこった はなびら（ぼろぼろ）*/
    ctx.fillStyle = PETAL2; ctx.strokeStyle = INK; ctx.lineWidth = 2.6;
    for (const [x, y, a] of [[-56, -152, -0.7], [50, -158, 0.5], [-20, -164, -0.1]]) {
      ctx.save(); ctx.translate(x, y); ctx.rotate(a);
      ctx.beginPath(); ctx.ellipse(0, 0, 13, 8, 0, 0, Math.PI * 2);
      ctx.fill(); ctx.stroke();
      ctx.restore();
    }
    ...
    if (cast > 0.1) {
      ctx.save(); ctx.globalAlpha = cast;
      ctx.fillStyle = '#7bd93b';
      for (let i = 0; i < 5; i++) {
        const a = t * 5 + i * 1.256;
        ctx.beginPath();
        ctx.arc(74 + i * 16 + cast * 16, -112 + Math.sin(a) * 30, 6 - i * 0.7, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
    }
...
/* サンズ（てき）── ピンチに なると ひだりめが ひかる（s.enraged）*/
function drawSansBoss(ctx, s) { drawSansBody(ctx, s, !!(s && s.enraged)); }
...
const DRAWERS = {
  ...
  /* ★とくせつ「地底の国」4〜7ステージ */
  vulkin: drawVulkin, muffet: drawMuffet,
  guard01: drawGuard01, guard02: drawGuard02,
  humansoul: drawHumanSoul,
  metaton_ex: drawMetatonEx, sans_boss: drawSansBoss,
  asgore: drawAsgore, flowey: drawFlowey,
  /* ★とくせつ「地底の国」（だい18しょう）*/
  toriel: drawToriel,
  papyrus: drawPapyrus,
  undyne: drawUndyne,
  froggit: drawFroggit,
  napstablook: drawNapstablook,
  lesserdog: drawLesserDog,
  snowdrake: drawSnowdrake,
  aaron: drawAaron,
  maddummy: drawMadDummy,
  haneneko: drawHaneneko,
  kyoudai: drawKyoudai,
  susube: drawSusube,
  ...
  only: drawOnly,
  olaf: drawOlaf,
  oharamen: drawOharaMen,
  nekooji: drawNekooji,
  nekooji_suit: drawNekoojiSuit,
  nekooji_old: drawNekoojiOld,
  nekooji_dr: drawNekoojiDr,
  zunio: drawZunio,
  zunita: drawZunita,
...
/* ==== こうさくアイテム もう 20しゅるい（v6.11）====
   かきかたは うえの 20こと おなじ。(0,0) が ゆかに つく ところ。 */

/* 21 れいぞうこ */
function drawFridge(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;  roundRect(ctx, -34, -120, 68, 120, 7); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -30, -116, 60, 112, 5); ctx.fill();
  ctx.fillStyle = p.dark;  ctx.fillRect(-30, -74, 60, 4);
  ctx.fillStyle = p.light; roundRect(ctx, 16, -108, 6, 26, 3); ctx.fill();
  roundRect(ctx, 16, -66, 6, 26, 3); ctx.fill();
  ctx.fillStyle = p.accent; roundRect(ctx, -22, -108, 18, 14, 3); ctx.fill();
}
/* 22 キッチン */
function drawKitchen(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;  roundRect(ctx, -58, -60, 116, 60, 5); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -54, -56, 108, 52, 4); ctx.fill();
  ctx.fillStyle = METAL;   roundRect(ctx, -60, -70, 120, 12, 4); ctx.fill();
  ctx.fillStyle = METALD;  roundRect(ctx, -44, -68, 30, 8, 3); ctx.fill();   // シンク
  ctx.fillStyle = '#455a64';
  for (const x of [8, 30]) { ctx.beginPath(); ctx.arc(x, -64, 5, 0, Math.PI * 2); ctx.fill(); }
  ctx.fillStyle = p.light; ctx.fillRect(-46, -40, 30, 4); ctx.fillRect(4, -40, 30, 4);
  ctx.fillStyle = METAL;   roundRect(ctx, -16, -82, 5, 14, 2); ctx.fill();   // じゃぐち
  roundRect(ctx, -30, -84, 16, 4, 2); ctx.fill();
}
/* 23 ピアノ */
function drawPiano(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;  roundRect(ctx, -62, -102, 124, 102, 6); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -58, -98, 116, 60, 5); ctx.fill();
  ctx.fillStyle = '#fafafa'; roundRect(ctx, -56, -38, 112, 16, 3); ctx.fill();
  ctx.fillStyle = '#222';
  for (let i = -50; i < 52; i += 8) ctx.fillRect(i, -38, 4, 10);
  ctx.fillStyle = p.light; roundRect(ctx, -46, -90, 92, 8, 4); ctx.fill();
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(0, -70, 7, 0, Math.PI * 2); ctx.fill();
}
/* 24 2だんベッド */
function drawBunkbed(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD;
  ctx.fillRect(-64, -122, 9, 122); ctx.fillRect(55, -122, 9, 122);
  ctx.fillStyle = WOODC; roundRect(ctx, -66, -34, 132, 16, 4); ctx.fill();
  roundRect(ctx, -66, -92, 132, 16, 4); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -60, -46, 120, 13, 4); ctx.fill();
  roundRect(ctx, -60, -104, 120, 13, 4); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -56, -56, 36, 12, 5); ctx.fill();
  roundRect(ctx, -56, -114, 36, 12, 5); ctx.fill();
  ctx.fillStyle = WOODD;   // はしご
  for (let y = -110; y < -20; y += 22) roundRect(ctx, 30, y, 30, 5, 2), ctx.fill();
}
/* 25 べんきょうづくえ */
function drawDesk(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD; ctx.fillRect(-52, -52, 10, 52); ctx.fillRect(42, -52, 10, 52);
  ctx.fillStyle = p.main; roundRect(ctx, -56, -66, 112, 15, 4); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, 4, -50, 48, 34, 3); ctx.fill();     // ひきだし
  ctx.fillStyle = p.light; roundRect(ctx, 14, -42, 28, 5, 2); ctx.fill();
  roundRect(ctx, 14, -28, 28, 5, 2); ctx.fill();
  ctx.fillStyle = p.accent; roundRect(ctx, -46, -78, 26, 12, 3); ctx.fill(); // ほん
}
/* 26 パソコン */
function drawPc(ctx, s) { const p = P(s);
  ctx.fillStyle = METALD; roundRect(ctx, -12, -18, 24, 8, 3); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, -34, -62, 68, 46, 5); ctx.fill();
  ctx.fillStyle = '#26c6da'; roundRect(ctx, -29, -57, 58, 36, 3); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.45)'; roundRect(ctx, -26, -54, 22, 8, 2); ctx.fill();
  ctx.fillStyle = p.main; roundRect(ctx, -30, -10, 60, 10, 3); ctx.fill();   // キーボード
  ctx.fillStyle = p.light;
  for (let i = -26; i < 26; i += 7) ctx.fillRect(i, -8, 5, 3);
}
/* 27 ゲームき */
function drawGameset(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;  roundRect(ctx, -34, -26, 68, 26, 5); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -30, -22, 60, 18, 4); ctx.fill();
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(-18, -13, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -6, -17, 30, 8, 3); ctx.fill();
  /* コントローラー */
  ctx.fillStyle = p.dark; roundRect(ctx, -26, -46, 52, 18, 8); ctx.fill();
  ctx.fillStyle = p.light; ctx.beginPath(); ctx.arc(-14, -37, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(14, -37, 4, 0, Math.PI * 2); ctx.fill();
}
/* 28 つみあげた ほん */
function drawBookpile(ctx, s) { const p = P(s);
  const cols = [p.main, p.dark, p.light, p.accent, p.main];
  let y = 0;
  for (let i = 0; i < 5; i++) {
    const w = 50 - i * 3, h = 11;
    ctx.fillStyle = cols[i];
    roundRect(ctx, -w / 2 + (i % 2 ? 4 : -4), y - h, w, h - 1, 2); ctx.fill();
    y -= h;
  }
}
/* 29 サボテン */
function drawCactus(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD; roundRect(ctx, -20, -26, 40, 26, 5); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, -22, -32, 44, 10, 4); ctx.fill();
  ctx.fillStyle = '#4caf50';
  roundRect(ctx, -10, -88, 20, 62, 10); ctx.fill();
  roundRect(ctx, -26, -70, 14, 30, 7); ctx.fill();
  roundRect(ctx, 12, -62, 14, 24, 7); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.35)';
  for (let i = 0; i < 5; i++) ctx.fillRect(-2, -84 + i * 12, 2, 6);
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(0, -92, 6, 0, Math.PI * 2); ctx.fill();
}
/* 30 はなびん */
function drawVase(ctx, s) { const p = P(s);
  ctx.fillStyle = p.main;
  ctx.beginPath(); ctx.moveTo(-14, 0); ctx.quadraticCurveTo(-20, -24, -9, -34);
  ctx.lineTo(9, -34); ctx.quadraticCurveTo(20, -24, 14, 0); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -10, -38, 20, 6, 3); ctx.fill();
  ctx.strokeStyle = '#4caf50'; ctx.lineWidth = 3; ctx.lineCap = 'round';
  for (const [dx, dy] of [[-12, -60], [0, -68], [13, -58]]) {
    ctx.beginPath(); ctx.moveTo(0, -36); ctx.quadraticCurveTo(dx * 0.5, -50, dx, dy); ctx.stroke();
  }
  ctx.fillStyle = p.accent;
  for (const [dx, dy] of [[-12, -60], [0, -68], [13, -58]]) {
    for (let i = 0; i < 5; i++) {
      const a = i / 5 * Math.PI * 2;
      ctx.beginPath(); ctx.arc(dx + Math.cos(a) * 5, dy + Math.sin(a) * 5, 3.4, 0, Math.PI * 2); ctx.fill();
    }
  }
}
/* 31 ちきゅうぎ */
function drawGlobe(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD; roundRect(ctx, -16, -10, 32, 10, 4); ctx.fill();
  ctx.fillStyle = METALD; ctx.fillRect(-3, -30, 6, 22);
  ctx.fillStyle = '#42a5f5'; ctx.beginPath(); ctx.arc(0, -50, 22, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#66bb6a';
  ctx.beginPath(); ctx.ellipse(-7, -56, 9, 6, 0.3, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(8, -44, 7, 5, -0.2, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = p.main; ctx.lineWidth = 4;
  ctx.beginPath(); ctx.arc(0, -50, 26, -0.4, Math.PI + 0.4); ctx.stroke();
}
/* 32 ブランコ */
function drawSwing(ctx, s) { const p = P(s);
  ctx.strokeStyle = WOODD; ctx.lineWidth = 7; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(-42, 0); ctx.lineTo(-20, -116); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(42, 0);  ctx.lineTo(20, -116); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(-26, -116); ctx.lineTo(26, -116); ctx.stroke();
  const sw = Math.sin((s && s.t || 0) * 1.6) * 6;
  ctx.strokeStyle = METALD; ctx.lineWidth = 2.6;
  ctx.beginPath(); ctx.moveTo(-14, -112); ctx.lineTo(-14 + sw, -44); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(14, -112);  ctx.lineTo(14 + sw, -44); ctx.stroke();
  ctx.fillStyle = p.main; roundRect(ctx, -22 + sw, -46, 44, 10, 4); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -18 + sw, -44, 36, 3, 2); ctx.fill();
}
/* 33 すべりだい */
function drawSlide(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD;
  ctx.fillRect(28, -92, 8, 92); ctx.fillRect(48, -92, 8, 92);
  ctx.fillStyle = p.dark;
  ctx.beginPath(); ctx.moveTo(-56, 0); ctx.lineTo(-40, 0);
  ctx.lineTo(40, -84); ctx.lineTo(22, -84); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.main;
  ctx.beginPath(); ctx.moveTo(-52, 0); ctx.lineTo(-42, 0);
  ctx.lineTo(38, -80); ctx.lineTo(26, -80); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, 20, -96, 40, 10, 3); ctx.fill();
  ctx.fillStyle = WOODD;
  for (let y = -78; y < -6; y += 18) { roundRect(ctx, 34, y, 26, 5, 2); ctx.fill(); }
}
/* 34 テント */
function drawTent(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;
  ctx.beginPath(); ctx.moveTo(-58, 0); ctx.lineTo(0, -92); ctx.lineTo(58, 0); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.main;
  ctx.beginPath(); ctx.moveTo(-50, 0); ctx.lineTo(0, -84); ctx.lineTo(50, 0); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.light;
  ctx.beginPath(); ctx.moveTo(-16, 0); ctx.lineTo(0, -56); ctx.lineTo(16, 0); ctx.closePath(); ctx.fill();
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(0, -90, 6, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = WOODD; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.moveTo(-58, 0); ctx.lineTo(-68, -8); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(58, 0);  ctx.lineTo(68, -8); ctx.stroke();
}
/* 35 ハンモック */
function drawHammock(ctx, s) { const p = P(s);
  ctx.fillStyle = WOODD;
  roundRect(ctx, -68, -80, 9, 80, 4); ctx.fill();
  roundRect(ctx, 59, -80, 9, 80, 4); ctx.fill();
  const sag = 26 + Math.sin((s && s.t || 0) * 1.3) * 3;
  ctx.fillStyle = p.main;
  ctx.beginPath();
  ctx.moveTo(-60, -72);
  ctx.quadraticCurveTo(0, -72 + sag + 16, 60, -72);
  ctx.quadraticCurveTo(0, -72 + sag, -60, -72);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = p.dark; ctx.lineWidth = 2;
  for (let i = -48; i <= 48; i += 12) {
    ctx.beginPath(); ctx.moveTo(i, -72 + sag * (1 - (i / 60) * (i / 60)) * 0.55);
    ctx.lineTo(i, -72 + (sag + 16) * (1 - (i / 60) * (i / 60)) * 0.95); ctx.stroke();
  }
  ctx.fillStyle = p.light; roundRect(ctx, -20, -76, 26, 8, 4); ctx.fill();   // まくら
}
/* 36 バスケゴール */
function drawHoop(ctx, s) { const p = P(s);
  ctx.fillStyle = METALD; roundRect(ctx, -6, -110, 12, 110, 4); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, -30, -12, 60, 12, 5); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -40, -132, 80, 46, 4); ctx.fill();
  ctx.strokeStyle = p.dark; ctx.lineWidth = 3;
  roundRect(ctx, -18, -112, 36, 20, 2); ctx.stroke();
  ctx.strokeStyle = '#ef6c00'; ctx.lineWidth = 3.4;
  ctx.beginPath(); ctx.ellipse(0, -88, 18, 5, 0, 0, Math.PI * 2); ctx.stroke();
  ctx.strokeStyle = 'rgba(255,255,255,.85)'; ctx.lineWidth = 1.6;
  for (let i = -14; i <= 14; i += 7) { ctx.beginPath(); ctx.moveTo(i, -86); ctx.lineTo(i * 0.5, -68); ctx.stroke(); }
}
/* 37 サッカーゴール */
function drawGoal(ctx, s) { const p = P(s);
  ctx.strokeStyle = p.light; ctx.lineWidth = 7; ctx.lineJoin = 'round';
  ctx.beginPath(); ctx.moveTo(-56, 0); ctx.lineTo(-56, -62); ctx.lineTo(56, -62); ctx.lineTo(56, 0); ctx.stroke();
  ctx.strokeStyle = 'rgba(255,255,255,.55)'; ctx.lineWidth = 1.4;
  for (let i = -50; i <= 50; i += 10) { ctx.beginPath(); ctx.moveTo(i, -60); ctx.lineTo(i, 0); ctx.stroke(); }
  for (let y = -54; y < 0; y += 10) { ctx.beginPath(); ctx.moveTo(-54, y); ctx.lineTo(54, y); ctx.stroke(); }
  ctx.fillStyle = p.main; ctx.beginPath(); ctx.arc(34, -12, 12, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.dark;
  for (let i = 0; i < 5; i++) { const a = i / 5 * Math.PI * 2;
    ctx.beginPath(); ctx.arc(34 + Math.cos(a) * 6, -12 + Math.sin(a) * 6, 2.6, 0, Math.PI * 2); ctx.fill(); }
}
/* 38 ロボットの おもちゃ */
function drawRobotoy(ctx, s) { const p = P(s);
  ctx.fillStyle = METALD; roundRect(ctx, -18, -14, 12, 14, 3); ctx.fill();
  roundRect(ctx, 6, -14, 12, 14, 3); ctx.fill();
  ctx.fillStyle = p.main; roundRect(ctx, -20, -52, 40, 40, 6); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, -12, -44, 24, 14, 3); ctx.fill();
  ctx.fillStyle = METAL;  roundRect(ctx, -30, -48, 10, 26, 4); ctx.fill();
  roundRect(ctx, 20, -48, 10, 26, 4); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -18, -78, 36, 28, 6); ctx.fill();
  ctx.fillStyle = '#263238';
  ctx.beginPath(); ctx.arc(-7, -66, 4, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(7, -66, 4, 0, Math.PI * 2); ctx.fill();
  ctx.fillRect(-8, -58, 16, 3);
  ctx.strokeStyle = METALD; ctx.lineWidth = 2.4;
  ctx.beginPath(); ctx.moveTo(0, -78); ctx.lineTo(0, -88); ctx.stroke();
  ctx.fillStyle = p.accent; ctx.beginPath(); ctx.arc(0, -91, 4, 0, Math.PI * 2); ctx.fill();
}
/* 39 きょうりゅうの ぬいぐるみ */
function drawDino(ctx, s) { const p = P(s);
  ctx.fillStyle = p.dark;
  ctx.beginPath(); ctx.moveTo(-14, -14); ctx.quadraticCurveTo(-48, -20, -40, -2);
  ctx.quadraticCurveTo(-28, -8, -12, -6); ctx.closePath(); ctx.fill();   // しっぽ
  ctx.fillStyle = p.main;
  ctx.beginPath(); ctx.ellipse(0, -30, 26, 24, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.light; ctx.beginPath(); ctx.ellipse(2, -24, 15, 14, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.main;
  ctx.beginPath(); ctx.ellipse(20, -56, 17, 14, 0.2, 0, Math.PI * 2); ctx.fill();   // あたま
  ctx.fillStyle = p.dark;
  for (let i = 0; i < 4; i++) {
    ctx.beginPath(); ctx.moveTo(-18 + i * 9, -48 + i * 3);
    ctx.lineTo(-13 + i * 9, -62 + i * 3); ctx.lineTo(-8 + i * 9, -47 + i * 3); ctx.closePath(); ctx.fill();
  }
  ctx.fillStyle = '#263238'; ctx.beginPath(); ctx.arc(27, -60, 3.2, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = p.dark; roundRect(ctx, -16, -12, 12, 12, 4); ctx.fill();
  roundRect(ctx, 6, -12, 12, 12, 4); ctx.fill();
}
/* 40 おおきな ケーキ */
function drawCake(ctx, s) { const p = P(s);
  ctx.fillStyle = '#fafafa'; roundRect(ctx, -34, -12, 68, 12, 5); ctx.fill();
  ctx.fillStyle = p.dark;  roundRect(ctx, -30, -34, 60, 24, 5); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -30, -40, 60, 10, 5); ctx.fill();
  ctx.fillStyle = p.main;  roundRect(ctx, -22, -58, 44, 20, 5); ctx.fill();
  ctx.fillStyle = p.light; roundRect(ctx, -22, -63, 44, 8, 4); ctx.fill();
  ctx.fillStyle = p.accent;
  for (const x of [-14, 0, 14]) { ctx.beginPath(); ctx.arc(x, -68, 4.4, 0, Math.PI * 2); ctx.fill(); }
  ctx.fillStyle = '#fff59d'; ctx.fillRect(-1.5, -82, 3, 14);      // ろうそく
  ctx.fillStyle = '#ff7043';
  ctx.beginPath(); ctx.ellipse(0, -86, 3.4, 5, 0, 0, Math.PI * 2); ctx.fill();
}

Object.assign(ROOM_DRAWERS, {
  fridge: drawFridge, kitchen: drawKitchen, piano: drawPiano, bunkbed: drawBunkbed,
  desk: drawDesk, pc: drawPc, gameset: drawGameset, bookpile: drawBookpile,
  cactus: drawCactus, vase: drawVase, globe: drawGlobe, swing: drawSwing,
  slide: drawSlide, tent: drawTent, hammock: drawHammock, hoop: drawHoop,
